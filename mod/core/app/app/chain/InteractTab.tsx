...
import {
  ...
  placeholderFor, readProvider, short, explorerUrl, txUrl,
} from './shared'
import { Panel, Label, Btn, Input, Empty, panelStyle } from './ui'
import type { ChainWallet } from './WalletBar'

export interface InteractTarget { name: string; address: string; abi: any[] }
...
export function InteractTab({
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (cancelled) return
      setFleet((abis.contracts || []).map((c: any) => ({ name: c.name, address: c.address, abi: c.abi })))
      setBuilds((mine.deployments || [])
        .filter((d: any) => d.abi?.length)
        .map((d: any) => ({ name: d.name, address: d.address, abi: d.abi })))
      setLoading(false)
    })
      ...
      <div style={{ marginBottom: '16px' }}>
        ...
        {showManual && (
          <Panel style={{ marginTop: '8px' }}>
            ...
            <Label style={{ marginTop: '10px' }}>ABI (JSON)</Label>
            <textarea
              value={manualAbi}
              ...
              style={{
                width: '100%', height: '120px', fontFamily: TERM_FONT, fontSize: '12px',
                padding: '8px', border: '1px solid var(--border-color)', background: 'transparent',
                color: 'var(--text-primary)', outline: 'none', resize: 'vertical',
              }}
            />
            <div style={{ marginTop: '10px' }}>
              <Btn size="sm" onClick={addManual}>LOAD</Btn>
            </div>
          </Panel>
        )}
      </div>
            ...
            <>
              ...
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap', marginBottom: '16px' }}>
                <Btn onClick={execute} disabled={busy || (!isRead(current) && !wallet.kind)}
                  color={isRead(current) ? READ : WRITE}>
