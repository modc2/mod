...
INSTRUCTIONS = (
    ...
    "one module's README + skill. docs_ask answers a natural-language question "
    'grounded in the docs (RAG, generated by a Liquid LFM). Everything is '
    'public — no auth.'
)
...
def _t_ask(args):
    ...
    if args.get('runtime'):
        kw['runtime'] = str(args['runtime'])
    return _docs().ask(_req(args, 'question'), **kw)
...
TOOLS = {
    ...
    'docs_ask': {
        'description': 'Ask a natural-language question about the mod protocol '
                       'and get a grounded answer: the relevant doc pages are '
                       'retrieved and an answer is generated from them by a '
                       'Liquid LFM. Returns the answer plus the pages it used as '
                       'sources. Use this for "how do I…/what is…" questions; use '
                       'docs_page/docs_search when you want the raw text yourself.',
        'inputSchema': {'type': 'object', 'properties': {
            'question': {'type': 'string', 'description': 'the question to answer'},
            'model': {'type': 'string', 'description': 'optional LFM model id (default from config)'},
            'runtime': {'type': 'string', 'enum': ['server', 'cloud'],
                        'description': 'optional generation runtime (default server)'},
        }, 'required': ['question']},
        'handler': _t_ask,
    },
}
