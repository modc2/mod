"""
copytensor.strats — modular strategy classes on the canonical Strat
schema shared with the polymarket and hyperliquid modules
(sync → signal → execute, tick, backtest — see base.py and README.md).
...
3. Implement `pick_leaders(ct)` — the copy-family defaults for
   `signal()`/`backtest()` then work as-is; override `signal()` (pure!)
   for logic that isn't mirroring.
4. Import + register the class below.

The venue-side live engine stays the sleeve-based CopyEngine
(src/engine/copier.py). A strat composes its sleeves via `build_copies`
— it doesn't replace the hot path.
"""
...
from .copy_coldkeys import CopyColdkeys
from .steady import Steady
from .top_n import TopN
from .whales import Whales

REGISTRY: Dict[str, type] = {
    CopyColdkeys.name: CopyColdkeys,
    TopN.name:         TopN,
    Whales.name:       Whales,
    Steady.name:       Steady,
}
...
def make(name: str, **kwargs: Any) -> Strat:
    """Instantiate a strat by name. Unknown names raise ValueError.
        ...
        make('top_n', n=5, days=7, capital=50)
        make('copy_coldkeys', ss58s=['5F3s…'])
    """
...
__all__ = [
    # canonical schema (shared with polymarket + hyperliquid)
    "Strat", "StratConfig", "Order", "OrderSide", "TraderTrade",
    "SyncResult", "ExecutionResult", "TickResult", "BacktestResult",
    # copytensor sleeve bridge
    "Leader", "StratParams", "flow_to_trade",
    # registry
    "CopyColdkeys", "TopN", "Whales", "Steady",
