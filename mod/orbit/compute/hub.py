...
CONFIRM_USD = float(os.environ.get('COMPUTE_CONFIRM_USD', '0.50'))
FANOUT_TIMEOUT = float(os.environ.get('COMPUTE_FANOUT_TIMEOUT', '30'))
...
def split_id(oid):
    ...
    if ':' not in s:
        raise ProviderError(
            ...
            f'ids come back from compute_search')
    name, ref = s.split(':', 1)
...
class Hub:
    ...
    def search(self, provider=None, kyc=None, sort='price', raw=True, **filters):
        f = P.Filters(**filters)
        ...
        if not raw:
            ...
            rows = [{k: v for k, v in o.items() if k != 'raw'} for o in rows]
        return {
            'filters': f.dict(),
            'count': len(rows),
    ...
    def rent(self, oid, hours=1.0, confirm=False, **opts):
        ...
        inst['hint'] = ('billing runs until compute_stop — '
                        f'`m compute/stop id={inst["id"]}`')
        return inst
