...
same `compute_search` that prices Akash prices the machine under your desk.

Off by default (`COMPUTE_LOCAL=1` to enable) because it touches docker and the
...
class Local(Provider):
    ...
    upstream = ''
    docs = 'm compute/local'
    kyc = 'none'
    pay = ('market tokens',)
    custody = 'local'
    caps = ('search', 'rent', 'instances', 'status', 'logs', 'exec', 'stop')
    key_hint = 'set COMPUTE_LOCAL=1 to include your own docker hosts'
    ...
    @property
    def enabled(self):
        return os.environ.get('COMPUTE_LOCAL', '') not in ('', '0', 'false')

    def _compute(self):
        if not self.enabled:
            raise ProviderError('local: disabled — set COMPUTE_LOCAL=1',
                                provider=self.name)
        if self._c is None:
            import importlib.util
            here = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            spec = importlib.util.spec_from_file_location(
                'compute_local_impl', os.path.join(here, 'compute.py'))
    ...
    def key_state(self):
        return 'enabled' if self.enabled else 'disabled (set COMPUTE_LOCAL=1)'
