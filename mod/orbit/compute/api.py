#!/usr/bin/env python3
"""compute api — REST + MCP + console on one port, zero dependencies.
...
`x-<provider>-key: …` headers, or one `x-compute-keys: {"lium":"…"}` header.
...
BASE = os.environ.get('BASE_PATH', '/compute')
PORT = int(os.environ.get('PORT', 50510))
...
def info():
    return {
        ...
        'byok': {'headers': ['x-<provider>-key: <key>',
                             'x-compute-keys: {"lium":"…","vast":"…"}'],
                 'keystore': '~/.mod/compute/keys.json (0600, off-tree)',
        ...
        'nodes': {
            ...
            'auth': 'owner-only: Authorization: Bearer `m compute/token`, or '
                    'call from localhost',
        },
    }
...
def _keys_from(headers):
    ...
    keys = {}
    blob = headers.get('x-compute-keys')
    if blob:
        try:
            got = json.loads(blob)
...
def serve(port=PORT, base=BASE):
    ...
    # mounted at /compute behind the gateway or served bare at :50510/.
    api_prefixes = (base.rstrip('/') + '/_api', '/api/compute', '/_api')

    class Handler(BaseHTTPRequestHandler):
        protocol_version = 'HTTP/1.1'
        server_version = 'compute/' + mcp.version()
        ...
        def _path(self):
            """Strip the gateway prefixes so /compute/_api/search == /search."""
