#!/usr/bin/env python3
"""redblue api — REST, the console and MCP on one port, stdlib only.
...
WHY WRITES ARE GATED AND READS ARE NOT
    ...
    Write `~/.mod/redblue/server.secret` to turn the gate on; with no secret file
...
PORT = int(os.environ.get('RB_PORT', 50970))
        ...
        **base,
        ...
        'auth': {
            ...
            'header': 'Authorization: Bearer <the contents of '
                      '~/.mod/redblue/server.secret>',
            'gated': sorted(WRITE_ROUTES),
        },
        'mcp': {'endpoint': 'POST /mcp', 'stdio': 'python3 -m redbluesrc.mcp',
...
def serve(port=PORT, bind=None, base=BASE):
    ...
    bind = bind if bind is not None else os.environ.get('RB_BIND', '0.0.0.0')
    # The console calls `<its own path>/_api`, so the same file works mounted
    # at /redblue behind the gateway and served bare on the port.
    api_prefixes = (base.rstrip('/') + '/_api', '/api/redblue', '/_api')
    corpus.seed_store()

    class Handler(BaseHTTPRequestHandler):
        protocol_version = 'HTTP/1.1'
        server_version = 'redblue/' + mcpsrv.version()
        ...
        def _path(self):
            """Strip the gateway prefixes so /redblue/_api/board == /board."""
    ...
    print(f'redblue on {bind}:{port} — api /, console {base}, mcp POST /mcp, '
