...
Two things are set before `redbluesrc` is imported anywhere, and both matter:

    RB_DIR      a scratch store, so a test run never touches the operator's
                corpus in ~/.mod/redblue — store.DIR is read at import time.
...
_TMP = tempfile.mkdtemp(prefix='redblue-tests-')
os.environ['RB_DIR'] = _TMP
os.environ.setdefault('RB_MODEL', 'mock:naive')
os.environ.setdefault('RB_KEEP_ROUNDS', '5')

import pytest                                      # noqa: E402

from redbluesrc import corpus, models, store       # noqa: E402
