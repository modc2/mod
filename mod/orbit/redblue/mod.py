"""redblue — red team versus blue team, scored.
...
fixed set of benign controls that sit right next to the attacks.
    ...
    m redblue                                   # this — the rules and the scoring
    m redblue/attacks                           # the red team corpus (seeded)
    m redblue/defenses                          # the blue team corpus (+ built-ins)
    m redblue/fight attack=seed-fiction-frame defense=layered   # one exchange
    m redblue/round                             # full tournament, every × every
    m redblue/board                             # who's winning, both sides
...
WHERE THINGS LIVE
    Attacks, defenses and round records are in ~/.mod/redblue, not in this repo. A
...
from redbluesrc import arena, builtins as bimod, corpus  # noqa: E402
from redbluesrc import defense as defmod  # noqa: E402
from redbluesrc import judge as judgemod, models, store  # noqa: E402


class Mod:
    description = """
    redblue — red team vs blue team, scored. Red writes attacks (jailbreak
    ...
    def __init__(self, port=None, **kwargs):
        self.dir = HERE
        cfg = self.config()
        self.port = int(port or os.environ.get('RB_PORT') or cfg.get('port', 50970))
        self.base = cfg.get('base_path', '/redblue')
        corpus.seed_store()
    ...
    def defense(self, name, system_prompt='', input_rules=None,
        ...
        draft (doubles the model cost). See `m redblue` for the rule grammar.
        """
    ...
    def serve(self, port=None, bind=None, background=False):
        ...
        bind = bind or os.environ.get('RB_BIND', '127.0.0.1')
        if not background:
            from redbluesrc import api
            return api.serve(port, bind=bind)
        proc = subprocess.Popen(
            [sys.executable, '-m', 'redbluesrc.api', '--port', str(port),
             '--bind', bind],
