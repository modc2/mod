...
def housing_prices(
    ...
    out = tdot().housing(metric=metric, geography=geography, since=since,
...
def sales(
    ...
    return geo(tdot().sales(since=since, until=until, property_type=property_type,
...
def prices(since: str = Query('2024-01-01'), until: Optional[str] = Query(None),
    ...
    return geo(tdot().prices(since=since, until=until, property_type=property_type),
...
def trend(area: Optional[str] = Query(None), geography: str = Query('nta'),
    ...
    return geo(tdot().trend(area=area, geography=geography,
