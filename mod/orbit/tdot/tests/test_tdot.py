...
from tdotgis import prices as P          # noqa: E402
...
def test_borough_lookup_accepts_aliases():
    import mod as m
    tdot = m.mod('tdot')()
    assert tdot.borough('bk')['name'] == 'Brooklyn'
    assert tdot.borough('Staten Island')['name'] == 'Staten Island'
    assert tdot.borough('The Bronx')['name'] == 'The Bronx'
    assert tdot.borough('Richmond')['name'] == 'Staten Island'
    assert 'error' in tdot.borough('Hoboken')


def test_housing_rejects_unknown_parameters():
    import mod as m
    tdot = m.mod('tdot')()
    assert 'error' in tdot.housing(geography='nowhere')
    assert 'error' in tdot.housing(metric='vibes')
...
def test_breaks_are_quantiles_and_monotonic():
    ...
    b = tdot._breaks(fc, 'median_price')
...
def test_price_change_breaks_are_symmetric_and_clipped():
    ...
    b = tdot._breaks(fc, 'price_change')
