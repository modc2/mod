...
`computefork_quote` / `computefork_rent` unchanged — the mod lane is a view, never a
...
HOST = os.environ.get('COMPUTE_FORK_MOD_HOST', '127.0.0.1')
TIMEOUT = float(os.environ.get('COMPUTE_FORK_MOD_TIMEOUT', '12'))
...
def lane(names=None, keys=None, compare=True, sample=6, **filters):
    ...
    return {
        ...
        'note': 'the same markets read through their own modules on this box; '
                'ids round-trip into computefork_quote / computefork_rent, which still '
