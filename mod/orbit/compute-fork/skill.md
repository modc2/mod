...
Port **50511** — API, console (`/compute-fork`) and MCP (`POST /mcp`) share it.

## Money rule

`computefork_rent` spends the caller's real credits and keeps spending until
`computefork_stop`. Before renting: run `computefork_quote` and **say the hourly number
...
the human has seen the price. After the work is done, `computefork_stop`; nothing
else ends the meter.

## The loop

```
m compute-fork/providers kyc=none        # who is reachable, who needs a key
m compute-fork/search gpu=H100 min_gpus=8 max_usd_hr=20
m compute-fork/quote id=vast:46095107 hours=6
m compute-fork/rent  id=vast:46095107 hours=6 confirm=1
m compute-fork/instances                 # everything running + combined burn rate
m compute-fork/logs id=vast:14002931
m compute-fork/stop id=vast:14002931
```

Search filters: `gpu` (loose — `4090` finds `NVIDIA GeForce RTX 4090`),
`min_gpus`, `min_vram_gb`, `max_usd_hr`, `region`, `provider`, `kyc`,
`kind` (gpu|cpu|confidential|job|storage|all), `sort` (price|vram|gpus), `limit`.

`computefork_search` never fails because one market is down — check the `providers`
...
| `local` | none | market tokens | your docker hosts; needs `COMPUTE_FORK_LOCAL=1` |
...
`m compute-fork/mods` (MCP: `computefork_mods`) reads those three markets *through those
...
`computefork_deploy`, and it works four ways:

```
m compute-fork/deploy docker=1                   # a container here — rents nothing
m compute-fork/deploy ssh=root@1.2.3.4           # a box you can already reach
m compute-fork/deploy id=clore:18022 confirm=1   # rent that offer, then install
m compute-fork/deploy instance=vast:14002931     # adopt a rental already running
```
...
```
m compute-fork/nodes                                   # state, mod version, burn rate
m compute-fork/node_sh   id=n-73e368 cmd='nvidia-smi'
m compute-fork/node_mods id=n-73e368                   # modules on that node
...
m compute-fork/node_push id=n-73e368 mod=compute       # send a local module over
m compute-fork/node_rm   id=n-73e368 release=1         # tear down AND stop the rental
```
...
`Authorization: Bearer $(m compute-fork/token)` or a request from localhost that did
...
`~/.mod/compute-fork/keys.json` → the sibling module's file (`~/.mod/lium/api_key`,
`~/.vast_api_key`, …). Set one with
`m compute-fork/set_key provider=vast key=… ` (written 0600, off-tree, never
...
`m compute-fork/raw provider=lium path=/volumes` calls that provider's own API with
...
`m compute-fork/serve` runs API + console + MCP under pm2 as `compute-api`. The
console's NODES tab is a terminal, a module browser and the install trail for
every node.
`m compute-fork/mcp_config` prints client config (stdio `python3 mcp.py`, or HTTP
`http://localhost:50511/mcp`). Zero dependencies — stdlib only.
