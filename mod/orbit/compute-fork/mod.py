"""compute-fork — one interface to every compute market.
...
hosts, behind one vocabulary:
    ...
    m compute-fork/search gpu=H100 max_usd_hr=3      # every market at once, cheapest first
    m compute-fork/quote id=lium:b7095b41 hours=4    # cost + what it costs elsewhere
    m compute-fork/rent id=lium:b7095b41 confirm=1   # spends YOUR credits
    m compute-fork/instances                         # everything running, with burn rate
    m compute-fork/stop id=lium:pod-…                # ends the billing

Same surface as MCP tools (`computefork_search`, `computefork_rent`, …) at POST /mcp,
so an agent and a human drive the identical code. Keys are per provider and
per caller: env, ~/.mod/compute-fork/keys.json, or the sibling module's key file —
this module never holds a house key.

`m compute-fork/serve` runs the API, the console and the MCP server on one port.
"""
...
class Mod:
    ...
    def __init__(self, keys=None, port=None, **kwargs):
        self.dir = HERE
        cfg = self.config()
        self.port = int(port or os.environ.get('PORT') or cfg.get('port', 50511))
        self.base = cfg.get('base_path', '/compute-fork')
        self._keys = keys or {}
    ...
    def deploy(self, id=None, ssh=None, instance=None, docker=None, name=None,
        ...
        """Rent (or adopt) a box and install the mod protocol on it.
            ...
            m compute-fork/deploy docker=1                    # a container, right here
            m compute-fork/deploy ssh=root@1.2.3.4            # a box you already have
            m compute-fork/deploy id=vast:46240433 confirm=1  # rent one, then install
        """
        import node
    ...
    def set_key(self, provider, key, persist=True):
        """Store one provider's key in ~/.mod/compute-fork/keys.json (0600, off-tree)."""
    ...
    def mcp_config(self):
        ...
        return {'mcpServers': {
            'compute-fork': {'command': 'python3', 'args': [os.path.join(HERE, 'mcp.py')]},
            'compute-fork-http': {'type': 'http', 'url': f'http://localhost:{self.port}/mcp'},
        }}

    # ── serve ────────────────────────────────────────────────────

    def serve(self, port=None, background=True, **kwargs):
        """Run API + console + MCP on one port, under pm2 as compute-fork-api."""
        ...
        subprocess.run(['pm2', 'start', sys.executable, '--name', 'compute-fork-api',
        ...
        return {'api': f'http://localhost:{port}',
                ...
                'process': 'compute-fork-api'}

    def kill(self, **kwargs):
        """Stop the server."""
        killed = []
        for name in ('compute-fork-api', 'compute-fork.api', 'compute-fork-app'):
