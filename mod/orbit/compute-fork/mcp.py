...
agent that learns `computefork_search` → `computefork_quote` → `computefork_rent` →
`computefork_stop` can drive Targon, Lium, Cathedral, Akash, Vast, Nosana, Prime,
...
Self-contained: JSON-RPC 2.0 hand-rolled on the stdlib, no `mcp` package.
    ...
    python3 mcp.py --http --port 50511 # Streamable HTTP — POST /mcp
...
INSTRUCTIONS = (
    ...
    'priced in USD/hr, addressed as provider:ref. Start with computefork_providers '
    '(who is reachable, who needs a key, who is no-KYC), then computefork_search '
    '(fan-out, cheapest first), computefork_quote (cost for N hours + cheaper '
    'alternatives), computefork_rent (spend-guarded), computefork_instances / '
    'computefork_logs, and computefork_stop — stopping is what ends the billing. '
    'Every call spends the caller\'s own credits on their own provider account; '
    'there is no shared key. computefork_raw is the escape hatch to a provider\'s '
...
_ID = _str('offer or instance id as provider:ref, e.g. lium:b7095b41 — '
           'ids come from computefork_search or computefork_instances')

TOOLS = {
    'computefork_providers': {
    ...
    'computefork_mods': {
        'description': 'The three markets that also ship as their own modules on '
                       ...
                       'upstream\'s own, so they round-trip into computefork_quote and '
                       'computefork_rent, which always go direct.',
        'inputSchema': {'type': 'object', 'properties': {
    ...
    'computefork_quote': {
        'description': 'What an offer costs for N hours, whether it trips the spend '
                       ...
                       'before computefork_rent — this is the only call that shows the '
    ...
    'computefork_rent': {
        'description': f'Rent an offer. SPENDS THE CALLER\'S MONEY and keeps spending '
                       f'until computefork_stop. Estimates over ${CONFIRM_USD} return a '
    ...
    'computefork_exec': {
        'description': 'Run a command inside a rental, where the provider supports it '
                       '(otherwise use the ssh line from computefork_status).',
        'inputSchema': {'type': 'object', 'properties': {
    ...
    'computefork_set_key': {
        'description': 'Store an API key for one provider in the off-tree keystore '
                       '(~/.mod/compute-fork/keys.json, 0600). Keys are never returned by '
    ...
    'computefork_deploy': {
        'description': 'Turn compute into a machine you can use: rent (or adopt) a '
                       ...
                       'money under the same spend guard as computefork_rent. The lite '
        ...
        'inputSchema': {'type': 'object', 'properties': {
            ...
            'wait': {'type': 'boolean',
                     'description': 'block until the install finishes (default false — '
                                    'it runs in the background and computefork_nodes shows '
                                    'progress)'},
        }},
        'handler': _t_deploy,
    },
    ...
    'computefork_node_mods': {
        'description': 'What the mod protocol on that node can do: every module it '
                       ...
                       'This is the map you read before computefork_node_call.',
        'inputSchema': {'type': 'object', 'properties': {
            'node': _str('node id'),
    ...
    'computefork_node_push': {
        ...
        'inputSchema': {'type': 'object', 'properties': {
            'node': _str('node id'), 'mod': _str('a local module name, e.g. compute-fork'),
...
def handle(body, owner=False):
    ...
    if method == 'initialize':
        ...
        return _result(id_, {
            ...
            'capabilities': {'tools': {}},
            'serverInfo': {'name': 'compute-fork', 'version': version()},
...
if __name__ == '__main__':
    ...
    if '--http' in argv:
        ...
        api.serve(int(argv[i]) if i > 0 else int(os.environ.get('PORT', 50511)))
    else:
        serve_stdio()
