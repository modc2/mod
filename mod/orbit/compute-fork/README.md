...
stranger with an SSH port. `compute-fork/deploy` finishes the job: it installs the
...
```
m compute-fork/search gpu=4090 max_usd_hr=1        # every market at once, cheapest first
m compute-fork/quote id=vast:46655560 hours=4      # cost, plus what it costs elsewhere
m compute-fork/rent  id=vast:46655560 confirm=1    # spends YOUR credits
m compute-fork/instances                           # everything running + combined burn rate
m compute-fork/stop  id=vast:14002931              # ends the billing

m compute-fork/deploy docker=1                     # a mod container here, in ~15s
m compute-fork/deploy id=vast:46655560 confirm=1   # rent one, install mod on it
m compute-fork/node_sh id=n-73e368 cmd=nvidia-smi  # a shell on it
m compute-fork/node_call id=n-73e368 mod=chain fn=balance
m compute-fork/node_rm id=n-73e368 release=1       # container down, rental stopped
```
...
1. **One vocabulary.** Every market answers in the same shape. `computefork_search`
...
2. **One caller's money.** Keys resolve per request, per provider: an explicit
   header, then the environment, then `~/.mod/compute-fork/keys.json` (0600), then
...
| `local` | your own docker hosts, billed on-chain (`COMPUTE_FORK_LOCAL=1`) | none | market tokens | full lifecycle |

`computefork_search kyc=none` restricts the fan-out to the permissionless half.
...
their own console, MCP tools and key file. `computefork_mods` reads those markets
**through those modules** and sets each answer beside what the direct lane reads
from the same upstream:

```
m compute-fork/mods                      # every mod-fronted market, both lanes
m compute-fork/mods mod=lium sample=10   # one of them, with offers
```
...
round-trips into `computefork_quote` and `computefork_rent` unchanged — the lane is a
...
wallet and will not pretend otherwise, so `computefork_rent` on those returns a
...
A market hands you an SSH line and a bill. `compute-fork/deploy` turns that into a
machine you can use, and keeps holding the rope afterwards.

```bash
m compute-fork/deploy docker=1 name=demo          # a container here — nothing rented
m compute-fork/deploy ssh=root@1.2.3.4            # a box you already have
m compute-fork/deploy id=clore:18022 confirm=1    # rent it, then install into it
m compute-fork/deploy instance=vast:14002931      # adopt a rental you already started
```
...
```bash
m compute-fork/nodes                                   # state, modules, burn rate
m compute-fork/node_sh   id=n-73e368 cmd='nvidia-smi'  # a shell
m compute-fork/node_mods id=n-73e368                   # what it can run
m compute-fork/node_mods id=n-73e368 mod=chain         # that module's fns
m compute-fork/node_call id=n-73e368 mod=chain fn=balance
m compute-fork/node_push id=n-73e368 mod=compute       # send it a module you just wrote
m compute-fork/node_rm   id=n-73e368 release=1         # tear down + stop the rental
```
...
```bash
m compute-fork/token          # the bearer token, minted into ~/.mod/compute-fork/server.secret
```
...
- `computefork_quote` is the only call that shows the **cross-market comparison** —
  run it before renting and you'll usually find the same card cheaper.
- `computefork_rent` **refuses** anything estimated above `COMPUTE_FORK_CONFIRM_USD`
  (default $0.50) unless called again with `confirm=true`. It returns the quote
  instead.
- `computefork_instances` reports the **combined burn rate** across every market.
- `computefork_stop` is what ends billing. Suspending doesn't. Closing a tab doesn't.

## MCP

```bash
m compute-fork/serve                    # api + console + mcp on :50511
m compute-fork/mcp_config               # drop-in client config
python3 mcp.py                     # stdio, for Claude Code / Desktop
curl -sX POST :50511/mcp -d '{"jsonrpc":"2.0","id":1,"method":"tools/list"}'
```

The console at `/compute-fork`, the REST routes, the CLI fns and the MCP tools all
call the same `Hub`, so they cannot drift apart.

## Keys

```bash
m compute-fork/set_key provider=lium key=…      # → ~/.mod/compute-fork/keys.json (0600)
m compute-fork/keys                             # which are set — never the keys
```

Also read (in this order): `x-lium-key:` / `x-computefork-keys:` request headers,
...
Aggregation always loses something. `computefork_raw` calls a provider's own API
...
```bash
m compute-fork/raw provider=lium path=/volumes
m compute-fork/raw provider=akash path=/v1/network-capacity
```
