...
USER_AGENT = 'mod-compute-fork/0.1 (mod protocol)'
TIMEOUT = 25
...
KEYS_FILE = os.path.expanduser('~/.mod/compute-fork/keys.json')
        ...
        if not k and required:
            raise NeedsKey(
                ...
                hint=self.key_hint or f'get one at {self.signup or self.upstream}, then '
                                      f'`m compute-fork/set_key provider={self.name} key=…`')
        return k or ''
...
class Filters:
    ...
    def __init__(self, gpu=None, min_gpus=None, min_vram_gb=None, max_usd_hr=None,
        ...
        self.limit = max(1, min(int(limit or 40), 200))
