...
LIVE = os.environ.get('COMPUTE_FORK_TEST_LIVE', '1') not in ('0', 'false')
...
def test_guard_lets_a_cheap_rent_through(monkeypatch):
    ...
    assert 'computefork_stop' in out['hint'] or 'stop' in out['hint']
...
def test_every_tool_is_declared_and_callable():
    ...
    for name, t in mcp.TOOLS.items():
        assert name.startswith('computefork_')
...
def test_initialize_echoes_a_protocol_version_we_implement():
    ...
    assert r['result']['serverInfo']['name'] == 'compute-fork'
...
def test_a_failing_tool_is_a_successful_response_carrying_iserror():
    r = mcp.handle({'jsonrpc': '2.0', 'id': 9, 'method': 'tools/call',
                    'params': {'name': 'computefork_status', 'arguments': {'id': 'junk'}}},
                   owner=True)
...
def test_unknown_tool_is_a_protocol_error():
    r = mcp.handle({'jsonrpc': '2.0', 'id': 9, 'method': 'tools/call',
                    'params': {'name': 'computefork_nope', 'arguments': {}}})
    assert r['result']['isError'] is True
...
@pytest.mark.skipif(not LIVE, reason='COMPUTE_FORK_TEST_LIVE=0')
