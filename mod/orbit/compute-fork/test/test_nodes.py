...
def test_pushing_a_module_sends_the_directory_as_it_is_on_this_disk():
    blob, arc = N.mod_tar('compute-fork')
    assert arc == 'mod/orbit/compute-fork'
    with tarfile.open(fileobj=io.BytesIO(blob)) as tar:
        names = tar.getnames()
    assert 'mod/orbit/compute-fork/mod.py' in names
    assert 'mod/orbit/compute-fork/node.py' in names
...
def test_an_unowned_mcp_call_cannot_deploy_or_spend():
    r = mcp.handle({'jsonrpc': '2.0', 'id': 1, 'method': 'tools/call',
                    'params': {'name': 'computefork_deploy', 'arguments': {'docker': True}}})
    ...
    r2 = mcp.handle({'jsonrpc': '2.0', 'id': 2, 'method': 'tools/call',
                     'params': {'name': 'computefork_providers', 'arguments': {}}})
    assert r2['result']['isError'] is False


def test_the_node_tools_are_declared_like_every_other_tool():
    for name in ('computefork_deploy', 'computefork_nodes', 'computefork_node_sh',
                 'computefork_node_mods', 'computefork_node_call', 'computefork_node_push',
                 'computefork_node_rm'):
        t = mcp.TOOLS[name]
...
def test_a_container_becomes_a_mod_node_and_answers_for_itself(registry):
    ...
    try:
        ...
        pushed = N.push(node['id'], 'compute-fork')
        assert pushed['bytes'] > 1000
        assert 'compute-fork' in N.ctl(node['id'], 'mods')['mods'], \
