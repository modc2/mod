 ...
 * @param {object} opts.files  the rest of this mod's folder, path → source,
 *                             importable by the anchor and by nothing else
...
export async function openClass(source, opts = {}) {
  const { seed = 1, seats = 0, timeoutMs = DEFAULT_CALL_TIMEOUT_MS, mcp = null, files = null } = opts;
  const { spawn } = await import("node:child_process");

  const python = process.env.MODARENA_PYTHON || "python3";
  ...
  try {
    ...
    // A mod is a folder: the anchor is `source`, and the other .py files in
    // the folder go with it so `import board` means this mod's board.py.
    const loaded = await send({ op: "load", source, seed, seats, files: files || {} },
                              Math.max(timeoutMs, 10_000));
    session.role = loaded.role;
    session.siblings = loaded.siblings || [];
