...
and, in the other direction, one request this process makes of the host:
    ...
    {"mcp": {"server": "modarena", "tool": "leaderboard", "arguments": {}}}
    → {"op": "mcp_result", "value": {...}}
...
# The other files in this mod's own folder, already executed, keyed by module
# name. A folder is a mod, so `import board` inside one means that folder's
# board.py — and can mean nothing else, because nothing else is in here.
_SIBLINGS = {}
...
def _guarded_import(name, globals=None, locals=None, fromlist=(), level=0):
    top = name.split(".")[0]
    if top in _SIBLINGS:
        return _SIBLINGS[top]
    if top not in ALLOWED_IMPORTS:
        known = ", ".join(sorted(ALLOWED_IMPORTS))
        mine = ", ".join(sorted(_SIBLINGS)) or "nothing"
        raise ImportError(
            f"the arena sandbox does not import `{name}` — a class here is pure computation. "
            f"Allowed: {known}. This mod folder also holds: {mine}."
        )
...
class Session:
    ...
    def load(self, msg):
        ...
        ns = _namespace(seed)
        # A mod is a folder, so the anchor may import the files beside it.
        # Each one is executed in its own namespace and handed to the import
        # guard as a module; the anchor then runs last, with them available.
        _SIBLINGS.clear()
        for path, text in sorted((msg.get("files") or {}).items()):
            if not path.endswith(".py") or "/" in path:
                continue
            name = path[:-3]
            module = types.ModuleType(name)
            module.__dict__.update(_namespace(seed))
        ...
        role = _role(self.obj)
        out = {"class": cls.__name__, "role": role, "siblings": sorted(_SIBLINGS),
