...
async function pyhost() {
  if (!IS_NODE) {
    throw new Error(
      ...
      "the runner instead: `m modarena/play game=… players=…`",
    );
  }
...
export function makeApi(base = "") {
  ...
  return {
    ...
    /**
     * The rest of a mod's folder — every file in it, text where the bytes are
     * text. The anchor still comes from `/blob`; this is what makes the files
     * *beside* the anchor real, which is the difference between a mod that is
     * a file and a mod that is a folder.
     */
    files: (id) => json(`/mods/${id}/files`),
...
async function loadModule(api, id) {
  ...
  if (lang === "python") {
    return { kind: "python", bytes: await api.blob(id), meta, lang, files: await siblings(api, id, meta) };
  }
  ...
  try {
    ...
    return { kind: "python", bytes, meta, lang: "python", files: await siblings(api, id, meta) };
  }
}

/**
 * The .py files in a mod's folder other than its anchor — what the sandbox
 * makes importable. A server that does not serve folders (or a mod that is
 * one file) answers with nothing, and a class that imports nothing never
 * notices either way.
 */
async function siblings(api, id, meta) {
  const anchor = meta?.anchor ?? "mod.py";
  if (meta && Array.isArray(meta.files) && meta.files.length < 2) return {};
  const folder = await api.files(id).catch(() => null);
  const out = {};
  for (const [path, body] of Object.entries(folder?.files ?? {})) {
    if (path === anchor || !path.endsWith(".py") || typeof body !== "string") continue;
    out[path] = body;
  }
  return out;
}
...
export async function openGame(api, id, { seed = 1, seats = 0, mcp = null } = {}) {
  const { kind, bytes, meta, lang, files } = await loadModule(api, id);
  if (kind === "wasm") {
  ...
  const session = await openClass(dec.decode(bytes), {
    seed, seats, files, mcp: mcp?.async ?? null,
  });
...
export async function driverFor(player, { api, seed, onEvent, mcp = null } = {}) {
  ...
  if (kind === "wasm" || kind === "class") {
    ...
    if (loaded.kind === "python") {
      ...
      const session = await openClass(dec.decode(loaded.bytes), {
        seed, files: loaded.files, mcp: mcp?.async ?? null,
      });
...
export async function runMatch({
  ...
  const door = mcp
    ? {
        allow: mcp.allow ?? [],
        sync: mcpFor(api.root, { allow: mcp.allow ?? [] }),
    ...
    while (turnNo < limit && !(await g.done())) {
      ...
      const asked = await Promise.all(
        active.map(async (seat) => {
          ...
          tally[seat].ms += ms;
          tally[seat].moves++;
          return { seat, view, move, raw, note, failed, ms, mcp: tally[seat].mcp - before };
        }),
      );
