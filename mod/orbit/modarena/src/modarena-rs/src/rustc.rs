...
/// How to build one. A class may take the seed, ignore it, or offer nothing at
/// all and rely on `Default`.
...
/// The whole crate, as one file. Kept as a function so the line arithmetic
/// below and the thing actually compiled can never disagree.
pub fn crate_source(source: &str, class: &Class) -> (String, usize) {
    let head = format!("{PRELUDE}\n// ── the class, exactly as it was uploaded ──\n");
...
"#;
    ...
    fn a_player_gets_play_and_nothing_else() {
        ...
        assert!(shim.contains("<Bot as Default>::default()"), "no `new`, so `Default` it is");
    }
