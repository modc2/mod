...
pub mod arena {
    ...
    ///
    /// ```ignore
    /// let reply = arena::mcp("modarena", "leaderboard", "{\"limit\": 3}");
    /// ```
...
#[macro_export]
macro_rules! log {
    ($($arg:tt)*) => { $crate::arena::log(format!($($arg)*)) };
}

// The class is compiled into this same module, so the bare name works too.
macro_rules! log {
