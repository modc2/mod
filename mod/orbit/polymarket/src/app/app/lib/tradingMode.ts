...
//   MODE        is the money real?       TEST · LIVE
//
...
export const MODE: Record<TradingMode, ModeMeta> = {
  TEST: {
    label: "TEST",
    dot: "○",
    meaning: "live feed, simulated fills — no orders are sent, no money moves",
    active: "TEST — the engine computes every mirror against the live feed and places none. No money can move.",
    pick: "Switch to TEST — the engine keeps computing mirrors but stops sending them. No money can move.",
  ...
  LIVE: {
    label: "LIVE",
    dot: "●",
    meaning: "real orders on Polymarket, funded by the trading wallet",
    active: "LIVE — mirrors are being sent to the CLOB as real orders against your wallet's USDC.",
    pick: "Switch to LIVE — mirrors start being sent to the CLOB as REAL orders against your wallet's USDC.",
...
export const MODE_LEGEND = `TEST ${MODE.TEST.dot} ${MODE.TEST.meaning}  ·  LIVE ${MODE.LIVE.dot} ${MODE.LIVE.meaning}`;
...
export function liveBlockedReason(canGoLive: boolean): string | null {
  return canGoLive
    ? null
    : "No capital behind this yet — fund the trading wallet (or give this trader an allocation) and LIVE unlocks.";
}
...
export function confirmGoLive(subject: string, amountUsd?: number | null): boolean {
  ...
  return window.confirm(
    `GO LIVE — real money.\n\n` +
      ...
      `taken back. Switching back to TEST stops new orders — it does not close ` +
...
export function describeSession(
  ...
  if (run === "STOPPED") {
    return {
      ...
      title: `Not running. It will start in ${mode} — ${MODE[mode].meaning}.`,
    };
  }
  if (run === "PAUSED" || opts?.paused) {
    return {
      ...
      chip: "border-amber-400/60 text-amber-400 bg-amber-400/[0.08]",
      title: `Polling is suspended. On resume it continues in ${mode} — ${MODE[mode].meaning}.`,
    };
  }
  return {
...
/** Roll-up phrasing for a desk of N sessions — "3 LIVE · 2 TEST". Kept here
...
export function describeFleet(running: number, live: number): string {
  ...
  if (live === 0) return `${test} in TEST`;
  if (test === 0) return `${live} LIVE`;
