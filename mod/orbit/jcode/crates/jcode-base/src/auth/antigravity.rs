...
// Upstream jcode embeds Google's Antigravity desktop-app OAuth credentials here.
// This vendored copy does not: push protection on this repo rejects any Google
// client id/secret pair, published or not. Supply them through the env vars below
// — see OAUTH.md. Antigravity login fails with instructions when they are unset.
...
fn env_credential(var: &str) -> Result<String> {
    std::env::var(var)
        .ok()
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "Antigravity OAuth is not configured: set {CLIENT_ID_ENV} and \
                 {CLIENT_SECRET_ENV} to a Google \"Desktop app\" OAuth client (see OAUTH.md)"
            )
        })
}

fn antigravity_client_id() -> Result<String> {
    env_credential(CLIENT_ID_ENV)
}

fn antigravity_client_secret() -> Result<String> {
    env_credential(CLIENT_SECRET_ENV)
}
...
mod tests {
    ...
    #[test]
    fn build_auth_url_errors_when_env_missing() {
        let _guard = lock_test_env();
        crate::env::remove_var(CLIENT_ID_ENV);
        ...
        .expect_err("no credentials are vendored, so this must fail");
        assert!(err.to_string().contains(CLIENT_ID_ENV));
    }

    #[test]
    fn blank_env_vars_count_as_unconfigured() {
        let _guard = lock_test_env();
