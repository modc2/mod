...
// Upstream jcode embeds the "Desktop app" OAuth credentials of Google's official
// Gemini CLI (@google/gemini-cli) here. This vendored copy does not: push protection
// on this repo rejects any Google client id/secret pair, published or not. Supply
// them through the env vars below — see OAUTH.md. Gemini OAuth login fails with
// instructions when they are unset; API-key auth is unaffected.
...
fn env_credential(var: &str) -> Result<String> {
    std::env::var(var)
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "Gemini OAuth is not configured: set {GEMINI_CLIENT_ID_ENV} and \
                 {GEMINI_CLIENT_SECRET_ENV} to a Google \"Desktop app\" OAuth client \
                 (see OAUTH.md), or authenticate with a Gemini API key instead"
            )
        })
}

fn gemini_client_id() -> Result<String> {
    env_credential(GEMINI_CLIENT_ID_ENV)
}

fn gemini_client_secret() -> Result<String> {
    env_credential(GEMINI_CLIENT_SECRET_ENV)
}
