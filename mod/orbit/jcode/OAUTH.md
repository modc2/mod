...
### OAuth client (required in this vendored copy)
Upstream jcode ships the Gemini CLI's own desktop OAuth client baked into the binary.
This copy does not — GitHub push protection on this repo rejects Google client
id/secret pairs, published or not — so you must bring your own:

```bash
export GEMINI_CLIENT_ID='<client-id>.apps.googleusercontent.com'
export GEMINI_CLIENT_SECRET='<client-secret>'
```

Create them as an OAuth client of type "Desktop app" in the Google Cloud console, or
reuse the credentials from your own `@google/gemini-cli` install. Without them
`jcode login --provider gemini` fails with this instruction; Gemini API-key auth
(`GEMINI_API_KEY` / `GOOGLE_API_KEY`) needs none of this.
...
- Env vars:
  - `JCODE_ANTIGRAVITY_CLIENT_ID` (**required** — no OAuth client is vendored in this copy; use a Google "Desktop app" client)
  - `JCODE_ANTIGRAVITY_CLIENT_SECRET` (**required**, as above; login fails with instructions when either is unset)
