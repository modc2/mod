"""
freewash-fork — a free-roam WebGL game set in Kensington Market, Toronto.

Walk or skate Augusta Avenue, Baldwin Street and Kensington Avenue: painted
Victorian storefronts, fruit crates on the sidewalk, vintage on the rails,
murals, hydro wires overhead and Bellevue Square in the middle of it. Rendered
entirely in the browser with WebGL (three.js). The whole game is a single
static HTML file in ``web/index.html``.
...
Usage:
    import mod as m
    m.mod('freewash-fork')().play()     # serve + open browser

    # or from the shell
    m freewash-fork play
    m freewash-fork serve port=8809 open=False
"""
import os
import threading
import webbrowser
...
def _make_handler(web_dir, mod):
    ...
    class _Handler(SimpleHTTPRequestHandler):
        ...
        def do_GET(self):
            # the gateway forwards /freewash-fork/* — serve those as if from root
            for pre in ("/freewash-fork", "/freewash_fork"):
                if self.path == pre or self.path.startswith(pre + "/"):
                    self.path = self.path[len(pre):] or "/"
                    break
...
class Mod:
    description = "Free-roam WebGL game in Kensington Market, Toronto — walk or skate. Renders in the browser via three.js/WebGL."

    fns = ["play", "serve", "deploy", "register", "stop", "url", "path", "test"]

    name = "freewash-fork"

    def __init__(self, web_dir: str = WEB_DIR, port: int = 8809, host: str = "127.0.0.1"):
    ...
    def serve(self, port: int = None, host: str = None, open: bool = False,
        ...
        if background:
            ...
            self._thread.start()
            print(f"freewash-fork serving at {self.url()}  (Ctrl-C / .stop() to halt)")
            if open:
                self._open()
            return self.url()
        else:
            print(f"freewash-fork serving at {self.url()}  (Ctrl-C to halt)")
            if open:
    ...
    def _start_relay(self, port: int):
        ...
        try:
            ...
            print(f"freewash-fork: multiplayer relay disabled ({e})")
            return
        ...
        async def main():
            ...
            for p in range(port, port + 25):
                try:
                    ...
                    continue
            if server is None:
                print("freewash-fork: no free port for multiplayer relay")
                return
            print(f"freewash-fork multiplayer relay on ws://{self.host}:{self._relay_port}")
        ...
        def run():
            ...
            try:
                ...
                print(f"freewash-fork relay stopped: {e}")
        ...
        try:
            ...
            print(f"freewash-fork: could not start relay ({e})")
    ...
    def stop(self):
        ...
        print("freewash-fork stopped")
        return {"ok": True}

    # -- gateway registration (modc2.com/freewash-fork) --------------------
    def register(self, app_url: str = None, owner: str = None,
                 gateway: str = "https://modc2.com"):
        """Register with the mod gateway so it routes ``/freewash-fork`` → this app.
        ...
        try:
            ...
            ns.reg("freewash-fork", url)
            ns.reg_app("freewash-fork", url, owner=owner, api_url=url)
            public = f"{gateway.rstrip('/')}/freewash-fork"
            print(f"freewash-fork registered → {public}  (app: {url})")
            return {"ok": True, "gateway": public, "app": url}
        except Exception as e:
            print(f"freewash-fork: gateway registration failed: {e}")
    ...
    def test(self):
        """Smoke test: the game file exists, is non-trivial, still renders
        through the pixel pipeline (low-res buffer + nearest upscale + dither),
        and is still Kensington Market rather than the park it was forked from."""
        ...
        # and so is the place: the street plan drives everything else in the
        # file, so if these are gone the fork has drifted back into the park
        for marker in ("const AUGUSTA", "KENSINGTON =", "BALDWIN =", "const SQUARE",
                       "buildShop", "shopRow", "buildPole", "wireRun", "Bellevue Square"):
            assert marker in head, f"Kensington plan missing: {marker!r}"
        assert "Washington Square" not in head, "this is the fork — the park should be gone"
        print(f"ok — {size} bytes, WebGL/three.js game present, "
              f"pixel pipeline intact, Kensington plan intact")
        return {"ok": True, "bytes": size, "pixelated": True, "place": "Kensington Market"}
