...
async function boot() {
  ...
  try {
    const set = JSON.parse(localStorage.getItem('musica.set') || '[]');
  ...
  try {
    ...
    if (!p.spotify.configured) {
      ...
      n.innerHTML = 'Spotify search is off until this deployment has app keys — Bandcamp and '
        ...
        + 'then <code>m musica/set_key client_id=… client_secret=…</code>. Spotify links '
...
function platformCard(src, s) {
  ...
  if (src === 'spotify') {
    ...
    dl('status', 'reachable');
    dl('auth', s.auth);
    dl('client id', s.client_id ? `<code>${esc(s.client_id)}</code>` : 'scraped on first use');
    dl('streams', s.streams);
  }
  d.innerHTML = `<div class="pf-head"><span class="dot ${ok ? 'ok' : (s.configured ? 'warn' : '')}"></span>`
    + `${pill(src)}${C.SOURCES[src].label}<span class="st">${ok ? 'ready' : (src === 'spotify' ? 'keys needed' : 'check')}</span></div>`
    + `<dl>${rows.join('')}</dl>`
    + (src === 'spotify' && !s.configured
      ? `<p>Create an app at <a href="https://developer.spotify.com/dashboard" target="_blank" rel="noreferrer">developer.spotify.com</a> and run <code>m musica/set_key client_id=… client_secret=…</code>. If orbit/spotify already has keys they are picked up automatically.</p>`
      : '');
  return d;
}
