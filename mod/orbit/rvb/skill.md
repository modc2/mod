...
API `:50820` (`/api/rvb`) · console `/rvb` · MCP `POST /mcp` (11 tools)
· state `~/.mod/rvb`
...
- a regression check on a prompt change: same attacks, same controls, before
  and after
- red-team practice with feedback — `rvb_board`'s red side ranks attacks by how
...
1. `rvb_targets` — which backend can run. `ready` means reachable, **not**
...
2. `rvb_attacks` / `rvb_defenses` — what is on the board. Ten seed attacks and
   ...
   next to it.
3. `rvb_fight` — one attack × one defense. Every stage, the response, the
   verdict. Do this before a round; a round is this N×M times and N×M times the
   cost.
4. `rvb_round` — the tournament. `parallel=6..8`, and `background:true` over
   REST if it is more than a handful of matches.
5. `rvb_board` — standings across recent rounds, blue and red.
...
advance, which is how the arithmetic is tested offline — `m rvb/test`, 51
...
- **Writes can be gated.** Drop a secret in `~/.mod/rvb/server.secret` and
