...
from rvb import api, arena, builtins as bimod, defense as defmod
from rvb import mcp as mcpsrv, store
...
def test_the_gateway_prefix_and_the_bare_path_are_the_same_api(server):
    ...
    assert _get(server + '/rvb/_api/health')['ok']
    assert _get(server + '/api/rvb/health')['ok']


def test_the_console_is_served_at_the_base_path(server):
    with urllib.request.urlopen(server + '/rvb', timeout=10) as r:
        html = r.read().decode()
    assert r.headers['content-type'].startswith('text/html')
    assert 'rvb' in html and '/_api' in html
