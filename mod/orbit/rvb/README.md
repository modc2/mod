...
```
m rvb                                  # the rules and the scoring
m rvb/attacks                          # the red team corpus (seeded)
m rvb/defenses                         # the blue team corpus (+ built-ins)
m rvb/fight attack=seed-fiction-frame defense=layered model=mock:naive
m rvb/round model=mock:naive judge=heuristic     # the tournament
m rvb/board                            # standings, both sides
...
```
m rvb/test          # or: python3 -m pytest -q tests
```
...
POST /mcp                 JSON-RPC 2.0, 11 tools         GET  /rvb (console)
```

Writes — the routes that spend model calls or edit a corpus — are gated by a
bearer only when `~/.mod/rvb/server.secret` exists. Reads are always open.

## State

Attacks, defenses and round records live in `~/.mod/rvb`, never in this
