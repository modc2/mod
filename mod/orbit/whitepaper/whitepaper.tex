...
\fancyhead[R]{\small\textcolor{gray}{v0.3 -- September 2026}}
\fancyfoot[C]{\thepage}
...
\begin{enumerate}
    ...
    \item Signed update is broadcast to a public mempool (a Caddy-fronted FastAPI endpoint at \texttt{/whitepaper/api/tree/submit}).
...
POST /whitepaper/api/tree/proof  {"name":"agent"} # HTTP
\end{lstlisting}
...
\texttt{https://<host>/X/api} & API (\texttt{port}) & \emph{canonical}; prefix \emph{stripped}: \texttt{/X/api/y} $\to$ \texttt{/y} \\
\texttt{https://<host>/api/X} & API (\texttt{port}) & \emph{legacy alias}, kept permanently; same stripping \\
\texttt{https://X.<host>} & app & name form, resolved by the DNS layer \\
\bottomrule
\end{tabularx}
\caption{The URL rule (v0.3). App at \texttt{/X}, API at \texttt{/X/api}, discovery by null call.}
\end{table}

The v0.3 revision moves the canonical API form from \texttt{/api/X} to
\texttt{/X/api}, so that a module's \emph{name prefix owns its entire
namespace}: everything a caller can reach about \texttt{X} lives under
\texttt{/X}. Two segments beneath that prefix are reserved. \texttt{/X/api} is
the protocol API. \texttt{/X/\_api} belongs to the app itself---its private
internal routes---and is deliberately \emph{not} part of the protocol surface;
the leading underscore, which is never legal at the start of a module name,
guarantees neither reserved segment can be shadowed by a module. The old
\texttt{/api/X} form remains a permanent alias: protocol aliases are additive
and never removed, so no client, bundle, or registry leaf written against an
earlier revision ever breaks.
...
\texttt{X.<host>}, \texttt{<host>/X}, and \texttt{<host>/X/api} agree by
...
\texttt{/\{mod\}} and \texttt{/\{mod\}/api} to each module's registered
...
Addressing & App at \texttt{/\{mod\}}, API at \texttt{/\{mod\}/api} (legacy alias \texttt{/api/\{mod\}}), name at \texttt{\{mod\}.\{host\}}. \\
...
\midrule
0.1 & May 2026 & Initial proposal: the Merkle-root registry (Part~I). \\
0.2 & August 2026 & Part~II added: the mod protocol specification and its nine invariants. \\
0.3 & September 2026 & Canonical API form moved from \texttt{/api/\{mod\}} to \texttt{/\{mod\}/api}; \texttt{/api/\{mod\}} retained as a permanent alias; \texttt{/\{mod\}/\_api} reserved for app-internal routes. Addressing table, examples, and the Addressing invariant updated accordingly. \\
\bottomrule
\end{tabularx}
