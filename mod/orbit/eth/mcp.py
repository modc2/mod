...
Self-contained JSON-RPC 2.0 on the stdlib — no `mcp` package.
    ...
    curl -s localhost:50730/mcp | jq  # the same schema, over HTTP
"""
...
def client_config(url=None):
    endpoint = url or CONFIG.get('urls', {}).get('mcp', 'http://localhost:50730/mcp')
    return {
        'http': {'mcpServers': {'eth': {'type': 'http', 'url': endpoint,
                                        'headers': {'Authorization': 'Bearer <mod-protocol token>'}}}},
        'stdio': {'mcpServers': {'eth': {'command': 'python3',
...
def describe(url=None):
    ...
    a client to see."""
    return {
        'server': {'name': 'eth', 'version': version(),
...
def handle(body, ctx=None):
    ...
    if method == 'initialize':
        ...
        return _result(id_, {
            ...
            'serverInfo': {'name': 'eth', 'version': version()},
            'instructions': INSTRUCTIONS,
        })
    if method == 'ping':
        return _result(id_, {})
...
if __name__ == '__main__':
    ...
    if '--tools' in argv or '--schema' in argv:
        ...
        port = int(argv[i]) if i > 0 else int(os.environ.get(
            'ETH_API_PORT', CONFIG.get('port', 50730)))
