...
const API = '_api';
const TOKEN_KEY = 'eth.token';
const THEME_KEY = 'eth.theme';
const NET_KEY = 'eth.network';
 ...
 */
async function buildToken(addr, data = { mod: 'eth' }) {
...
/** One way in to every tab, so the bench can send you to one — "interact with
 *  what you just deployed" is a link between two panes, not a click to hunt. */
function showTab(pane) {
  document.querySelectorAll('#tabs .tab').forEach((t) =>
    t.classList.toggle('on', t.dataset.pane === pane));
  document.querySelectorAll('.pane').forEach((p) =>
    p.classList.toggle('on', p.id === pane));
  if (pane === 'pane-agents') loadMcp();
  if (pane === 'pane-contracts') loadContracts();
...
async function refresh() {
  ...
  $('who').textContent = address ? short(address) : 'not signed in';
  $('signin').textContent = address ? 'sign out' : 'sign in';
  $('signin').onclick = address ? signOut : signIn;
...
function fillNetworks(list) {
  ...
  select.innerHTML = list.map((n) =>
    `<option value="${esc(n.name)}"${n.name === chosen ? ' selected' : ''}>${esc(n.name)}${n.testnet ? '' : ' ·'}</option>`
  ).join('');
  network = chosen;
...
async function openContract(addr, { jump = false } = {}) {
  const card = $('interact-card');
  if (jump) showTab('pane-contracts');
  try {
...
const bench = initBuild({
  ...
  accounts: () => accounts,
  signIn,
  openContract: (addr) => openContract(addr, { jump: true }),
...
setInterval(() => { if (document.visibilityState === 'visible') renderGas(); }, 30000);
