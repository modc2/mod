...
Run:
    uvicorn api.api:app --host 0.0.0.0 --port 50730
    python3 api/api.py --port 50730
"""
...
BASE = CONFIG.get('base_path', '/eth')

app = FastAPI(title='eth', version=VERSION,
...
async def api_alias(request: Request, call_next):
    """`/eth/_api/*` is this same API, one path segment along.
...
@app.get('/health')
def health():
    return {'ok': True, 'service': 'eth', 'time': int(time.time())}
...
class ProjectBody(BaseModel):
    ...
    # Where this came from, when it came from somebody. Editing a bundle you
    # opened by CID and saving it is a fork by another route, and the copy
    # should carry the same provenance the /fork route records.
...
def update_project(project: str, body: ProjectBody,
    ...
    return projects.save(who, _token(authorization), project=project,
                         ...
                         public=body.public, origin_cid=body.origin_cid)
...
def _mcp_url(request: Request) -> str:
    ...
    Behind the console the page asked its own origin for `/eth/_api/...`, which
...
if __name__ == '__main__':
    ...
    parser.add_argument('--port', type=int,
                        default=int(CONFIG.get('port', 50730)))
