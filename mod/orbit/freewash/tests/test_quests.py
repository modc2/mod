...
with sync_playwright() as p:
    ...
    pg.goto("http://127.0.0.1:8799/index.html?mute=1", wait_until="load")
    ...
    if inv:
        ...
        check("invitee is walking to the busker", sent and sent["mode"] == "walk",
              str(sent) + " busker at (-21,25)")
        if sent:
            check("...and heading to the right patch of park",
                  math.hypot(sent["tx"]+21, sent["tz"]-25) < 3.5)
    ...
    print("\n-- lap: a fountain lap on the board --")
    take("lap")
    ...
    pg.evaluate("""() => { const f=window.__fw; f.state.skating=false;
        for(let i=0;i<40;i++){ const a=i/40*6.283; f.goto(Math.cos(a)*20, Math.sin(a)*20); } }""")
    ...
    for i in range(30):
        a = i/29*6.283
        pg.evaluate(f"""() => {{ const f=window.__fw, a={a};
            f.state.skating = true;
            f.state.pos.set(Math.cos(a)*20, 0, Math.sin(a)*20);
    ...
    pg.evaluate("""() => { const f=window.__fw; const d = f.dogs.find(x=>!x.userData.ai.penned);
        d.position.set(-62, 0, 24); d.userData.ai.pos.set(-62, 0, 24);
        f.goto(-62, 25.6); window.__d = d; }""")
    pg.wait_for_timeout(900)
    check("dog follows once you reach it", pg.evaluate("!!window.__d.userData.ai.followHero"))
    # walk it in through the gate
    for _ in range(60):
        pg.evaluate("window.__fw.goto(-62, 18)")
    ...
    check("dog ends up penned in the run", penned["pen"] and not penned["follow"], str(penned))
