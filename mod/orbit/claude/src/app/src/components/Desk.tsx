...
// Desk — a browser-OS window manager for module apps. Each window iframes a
...
// so embedded apps keep their state.
...
const STORE_KEY = "claude.desk.v1";
...
export default function Desk({
  ...
  // into cells[0], the rest render as "pick an app" slots.
  launch: { mod: string; n: number; cells?: Frac[] } | null;
  ...
  const deskSize = () => {
    ...
    return { W: el?.clientWidth || window.innerWidth, H: (el?.clientHeight || window.innerHeight) - TASKBAR_H };
  };
      ...
      if (raw) {
        ...
        if (Array.isArray(data?.wins)) {
          const restored: Win[] = data.wins
            ...
            // One window per mod, and ids must match openWin's `w-${mod}`
            // scheme so a SPLIT for a restored mod raises it, not a dupe.
            .filter((w: any, i: number, a: any[]) => a.findIndex((x) => x.mod === w.mod) === i)
            .slice(0, 12)
            .map((w: any, i: number) => ({
              id: `w-${w.mod}`,
              mod: w.mod,
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(() => {
      try {
        const slim = wins.map((w) => ({ mod: w.mod, rect: w.rect, snap: w.snap, min: w.min }));
  ...
  // Windows are one-per-mod (openWin dedupes), so the id can be the mod
  // name itself. That keeps openWin idempotent — StrictMode double-fires
  // the SPLIT launch effect in dev, and the second call must land on the
  // same window — and lets focus be set without reading state back out of
  // a setWins updater (updaters run during render; ids computed inside one
  // can't be read reliably after the call).
  ...
  useEffect(() => {
    ...
    if (launch.cells && launch.cells.length > 0) {
      snapTo(`w-${launch.mod}`, launch.cells[0]);
      setPending(launch.cells.slice(1));
      setPendFilters({});
      setAssist(null);
    }
