#!/usr/bin/env python3
"""store_bridge — publish one claude task into localfs + the store index.
...
"key": "claude-task-xxxx.json"}. The task bundle is put into localfs (the
...
def main() -> int:
    ...
    key = envelope.get("key") or "claude-task.json"
