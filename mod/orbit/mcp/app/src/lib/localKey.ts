/**
 * Local sign-in — a keypair the browser derives and holds, for people with no
 * wallet extension. The hub only ever verifies a signature over the protocol
 * token envelope, so an ethers wallet minted here is indistinguishable from
 * MetaMask to the server: same address space, same `personal_sign` format.
 *
 * Trade-offs the UI states plainly on the sign-in card:
 ...
 *     origin) can read it — a throwaway publishing identity, not a vault;
 *   - clearing site data destroys it, and with it your claim on every server
 *     slug you published. `exportLocalKey` / `importLocalKey` are the backup.
 */
 ...
 * this tab, which the caller warns about before anything is published.
 */
...
/** Permanent: without a backup, every slug published under this key is orphaned. */
