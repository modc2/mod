...
A fork of [`eth`](../eth) with the console rebuilt. Same engine, same API,
same 42 MCP tools, same `~/.mod/eth` — the accounts, deployments and projects
you already have show up here — with a chrome that behaves like an app:

* **one sticky bar** holding the chain, who you are and the six sections, all
  of it still on screen after you scroll a 400-line contract;
* **a chain picker that says what it costs** — a testnet is a quiet pill, a
  real chain turns the whole control gold and hangs `REAL MONEY` off it, which
  is a thing you want to see before you press deploy and not after;
* **a heartbeat** — the latest block and the current base fee, ticking in the
  bar, so the page proves the chain is live without being asked;
* **an address you can recognise** — the identity pill draws a picture out of
  your own address bytes and copies it on click;
* **shortcuts** — `ctrl/cmd + enter` compiles, `ctrl/cmd + S` saves;
* light and dark on one palette, and a bar that truncates instead of wrapping
  at any width down to a phone.
...
```
console   http://localhost:50751/ethdesk    · plain ES modules, no build step
api       http://localhost:50750           · FastAPI, mod-protocol auth
cli       m ethdesk/test project=my-token account=dev network=base-sepolia
mcp       POST http://localhost:50750/mcp   · or `python3 mcp.py` on stdio
```
...
- **a fork is honest.** Opening somebody's CID gives you a read-only view and a
  fork button. The copy is yours from the first byte, with `origin_cid`
...
```bash
./serve.sh                     # pm2: ethdesk-api (:50750) + ethdesk-app (:50751)
anvil                          # optional: a free local chain on :8545
```

Then open <http://localhost:50751/ethdesk> and sign in with a browser wallet, or:
...
api/api.py      FastAPI — the API on :50750
app/            the console (:50751)
  ...
  server.py       static files + a proxy for /ethdesk/_api
...
|---|---|
| `ETH_DIR` | state dir — **defaults to `~/.mod/eth`, i.e. shared with the `eth` module**, which is why your accounts and deployments are already here; set it to split them |
...
| `ETHDESK_API_PORT` / `ETHDESK_APP_PORT` | ports (50750 / 50751) |
