...
APP = os.environ.get('ETHDESK_APP_URL', 'http://127.0.0.1:50751/ethdesk/')
OUT = Path(os.environ.get('ETHDESK_SHOT_DIR', '/tmp/ethdesk-shots'))
OUT.mkdir(parents=True, exist_ok=True)

TOKEN = auth().token({'mod': 'ethdesk'})
...
def shot(browser, tab, theme, token=None, label=None):
    ...
    page.add_init_script(f"""
        try {{
            localStorage.setItem('ethdesk.theme', {theme!r});
            localStorage.setItem('ethdesk.network', 'local');
            {f"localStorage.setItem('ethdesk.token', {token!r});" if token else
             "localStorage.removeItem('ethdesk.token');"}
        }} catch (e) {{}}
    """)
