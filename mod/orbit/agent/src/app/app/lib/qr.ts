...
//
// We render prompt-share CIDs as QR codes locally instead of calling an
// external QR-image service, so library content never leaves the browser.
// This is a compact, faithful implementation of the QR spec sufficient for
// short payloads like localfs CIDs (~200 chars max at level M / version 10).
// Output is a boolean matrix (true = dark module); callers render it (qrSvg).
...
function applyMask(
  ...
  for (let r = 0; r < size; r++) {
    for (let c = 0; c < size; c++) {
      ...
      if (!reserved[r][c] && maskFn(mask, r, c)) v = !v;
      out[r][c] = v;
    }
  }
  return out;
