"""Server for the supermario emulator: static bundle + the mod protocol API.

Kept as a standalone script so pm2 can run it directly (``python3 serve.py``)
without importing the orbit loader.

One process answers both halves of the protocol's URL rule, because the
emulator has no state to keep and no reason to burn a second port:

    /supermario/*        → the app   (prefix kept; stripped here)
    /api/supermario/*    → the API   (prefix stripped by the gateway, so the
                                       protocol routes land at the root)

    python3 serve.py [--port 50342] [--host 0.0.0.0]
"""
...
def module():
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            'supermario_mod', os.path.join(MODULE_DIR, 'mod.py'))
        anchor = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(anchor)
        _mod = anchor.Mod()
    except Exception as e:                  # the emulator still serves without it
        _mod_error = f'{type(e).__name__}: {e}'
        sys.stderr.write(f'supermario: API unavailable — {_mod_error}\n')
    finally:
        sys.path[:0] = shadow
    return _mod
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def _strip(self):
        ...
        The bare ``/supermario`` must NOT redirect to ``/supermario/``: the
    ...
    def _route(self) -> str:
        ...
        ``/{fn}``, ``/mod/supermario/{fn}``, and a null ``/mod/supermario``
    ...
    def end_headers(self):
        # the emulator is one static bundle; keep browsers from pinning an old
        # build, and let a page on another origin talk to the API
...
def main():
    ...
    ap.add_argument('--port', type=int, default=int(os.environ.get('PORT', 50342)))
    ap.add_argument('--host', default=os.environ.get('HOST', '0.0.0.0'))
    args = ap.parse_args()

    if not os.path.exists(os.path.join(WEB_DIR, 'index.html')):
        raise SystemExit('supermario: web/index.html is missing')

    srv = ThreadingHTTPServer((args.host, args.port), Handler)
    print('supermario serving %s on http://%s:%d%s (api: /api/%s)'
