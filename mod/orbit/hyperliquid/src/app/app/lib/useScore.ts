...
// The hook behind the board's ƒ SCORE box. Give it the text of the box and
// the board's window length and it hands back a synchronous scoreFor covering
// all three languages: plain expressions and JS compile inline; Python
// compiles through Pyodide (async, debounced so half-typed defs don't spam
// the compiler) and lands as state.
//
...
import type { TopTrader } from "./api";
import {
...
export function useCompiledScore(formula: string, days: number): ScoreState {
  ...
  const scoreFor = useCallback(
    (t: TopTrader): number | null =>
      compiled.fn ? compiled.fn(scoreInputs(t, days)) : Number.NEGATIVE_INFINITY,
    [compiled, days],
  );
