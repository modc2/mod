...
- `GET /trader/{address}/curve?days=7` - The window's PnL as a curve: `points` are `[ms, cumulative pnl]` rebased so the window opens at zero, taken from Hyperliquid's own portfolio series (realised **and** unrealised — the same definition the leaderboard prices `pnl` with, so a row's number and its curve agree). Also `max_drawdown` / `max_drawdown_pct`, the deepest peak-to-trough fall inside the window. Cheap (one upstream call, cached 5 min) and infallible: upstream trouble answers `available: false` with a `note` rather than a status code. This is what the board draws over the address when you hover a row.
