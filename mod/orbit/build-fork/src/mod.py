...
class Mod:
    ...
    Operates in two modes:
      - Server mode: delegates to Rust job server (port 8894) + Next.js app (8895)
    ...
    def __init__(self, api_url: str = None, app_url: str = None,
        ...
        name = cfg.get('name', 'build-fork')
        self.api_url = api_url or servers.get(f'{name}-api', 'http://localhost:8894')
        self.app_url = app_url or servers.get(f'{name}-app', 'http://localhost:8895')
        self.config = cfg
        ...
        # the other private state (~/.mod/build-fork/). Overridable in tests.
        self._prompts_dir = Path(os.path.expanduser('~/.mod/build-fork'))
    ...
    def register_onchain(self, network: str = None, data: str = None,
        ...
        try:
            if not force and chain.mod_exists('build-fork'):
                existing = chain.get_mod(chain.name2id('build-fork'))
                if existing.get('data') == data:
                    return {'status': 'already_registered', 'name': 'build-fork', 'data': data}
        except Exception:
            pass
        receipt = chain.reg('build-fork', data)
        tx = receipt.transactionHash.hex() if hasattr(receipt, 'transactionHash') else str(receipt)
        return {'status': 'registered', 'name': 'build-fork', 'data': data, 'tx': tx}
    ...
    # off-tree in ~/.mod/build-fork/whitelist.json (never committed) — the same file
    ...
    def _whitelist_path(self) -> Path:
        return Path(os.path.expanduser('~/.mod/build-fork/whitelist.json'))
    ...
    def _request(self, method: str, path: str, data: dict = None, timeout: int = 30,
        ...
        try:
            ...
            raise ConnectionError(
                ...
                f"start it with: cd mod/orbit/build-fork/api && cargo run\n{e}"
            )
    ...
    def _start_api(self) -> bool:
        ...
        binary = os.path.join(api_dir, 'target', 'release', 'build-fork-jobs')
    ...
    def kill(self, pid: int = None, port: int = None, sig: str = "SIGKILL") -> dict:
        ...
        if pid is None and port is None:
            killed = []
            api_port = self.config.get('port', 8894)
            app_port = self.config.get('app_port', 8895)
    ...
    def mr_merge(self, id: str, instructions: str = None, model: str = None, key=None) -> dict:
        ...
        if target and target != 'build-fork':
    ...
    def ensure_env(self, app_dir: str = None, api_dir: str = None):
        ...
        # ── API: Rust binary ──
        binary = api_dir / 'target' / 'release' / 'build-fork-jobs'
    ...
    def _serve_pm2(self, api_port, app_port, dev, results) -> bool:
        """Launch build-fork-api + build-fork-app under pm2 (the root deployment path).
        ...
        api_bin = module_dir / 'src' / 'api' / 'target' / 'release' / 'build-fork-jobs'
        if not eco.exists():
            return False
        ...
        env['BUILD_FORK_API_PORT'] = str(api_port)
        env['BUILD_FORK_APP_PORT'] = str(app_port)
        # NOTE: deliberately NOT setting BUILD_FORK_JOBS_LOCAL — as root we must keep
        # auth + the sudo gate on.
        subprocess.run(['pm2', 'delete', 'build-fork-api', 'build-fork-app'],
                       capture_output=True, text=True)
    ...
    def serve(self, api_port=None, app_port=None, dev=True):
        ...
        """
        api_port = int(api_port or self.config.get('port', 8894))
        app_port = int(app_port or self.config.get('app_port', 8895))
        log_dir = Path('/tmp/build-fork')
        ...
        # Frontend talks to the core gateway at /api/build-fork, not the API directly.
        # NEXT_PUBLIC_API_PORT is only used for in-app start/stop service mgmt.
        app_env['NEXT_PUBLIC_BASE_PATH'] = '/build-fork'
        ...
        # ── Register in app namespace so gateway routes /build-fork → here ──
        try:
            registry = m.mod('server.namespace')()
            registry.reg('build-fork', api_url)
...
"""
    ...
    def mod_register(self, mod: str, comment: str = '') -> dict:
        ...
        api/reg POST that build-fork-jobs makes today — same on-the-wire shape."""
