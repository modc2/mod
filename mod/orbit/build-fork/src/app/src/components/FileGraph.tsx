...
// clickable child list. Each node is a chip that carries its own name — mark
// and label inside one tinted, type-colored capsule, so a node is a single
// object to hit and to read. Columns are as wide as their widest chip and
// edges are recessive hairlines running edge-to-edge between them, so the
...
interface PlacedNode {
  ...
  size?: number | null;
  depth: number;
  badge: string; // count/(＋) suffix drawn inside collapsed folder chips
  w: number; // chip width (label-fitted)
  x: number; // chip left edge
  y: number; // chip vertical centre
  parent?: PlacedNode;
}
...
export default function FileGraph({
  ...
  const { nodes, edges, width, height, dirCount, fileCount } = useMemo(() => {
    ...
    const layout = (node: FileGraphNode, depth: number, parent?: PlacedNode): PlacedNode => {
      ...
      const kidsKnown = !isDir || kids.length > 0 || grafts.has(node.path);
      const badge =
        !isDir || isOpen ? "" : !kidsKnown ? " ＋" : stats && stats.files > 0 ? ` ${stats.files}` : "";
      ...
      const self: PlacedNode = {
        ...
        depth,
        badge,
        w: chipWidth(label, badge),
        x: 0, // filled in once column widths are known
        y: 0,
        parent,
      };
      if (isOpen) {
        ...
        self.y = PAD_Y + NODE_H / 2 + leafRow * ROW_H;
        leafRow++;
      }
      placed.push(self);
      return self;
    ...
    const root: PlacedNode = {
      ...
      depth: 0,
      badge: "",
      w: chipWidth(truncateLabel(rootLabel), ""),
      x: PAD_X,
      y: PAD_Y + NODE_H / 2,
    };
    ...
    // Columns are as wide as their widest chip, so labels never collide with
    // the next depth and shallow trees don't waste half the canvas.
    const colW: number[] = [];
    for (const n of placed) colW[n.depth] = Math.max(colW[n.depth] || 0, n.w);
    const colX: number[] = [PAD_X];
    for (let d = 1; d <= maxDepth; d++) colX[d] = colX[d - 1] + (colW[d - 1] || 0) + COL_GAP;
    for (const n of placed) n.x = colX[n.depth];
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden min-h-0">
      ...
      >
        ...
        <span className="text-[11px] font-code hidden sm:inline" style={{ color: "var(--text-tertiary)", opacity: 0.6 }}>
          ...
          <div className="relative" style={{ width: width * scale, height: height * scale }}>
            ...
            >
              ...
              {edges.map((e) => {
                ...
                const x2 = e.to.x;
                const midX = (x1 + x2) / 2;
                const lit = hover && (hover.path === e.to.path || hover.path === e.from.path);
                return (
                  <path
                    key={`${e.from.path}→${e.to.path}`}
                    d={`M ${x1} ${e.from.y} C ${midX} ${e.from.y}, ${midX} ${e.to.y}, ${x2} ${e.to.y}`}
                    fill="none"
                  ...
                  >
                    ...
                    <rect
                      ...
                      style={{
                        ...
                        stroke: isSelected
                          ? "var(--crt-green)"
                          : `color-mix(in srgb, ${color} ${isHover ? 80 : 42}%, transparent)`,
            ...
            {hover && !hover.isRoot && (
              <div
                ...
                style={{
                  left: Math.min((hover.x + 12) * scale, Math.max(0, width * scale - 260)),
                  top: (hover.y + NODE_H / 2 + 8) * scale,
                  maxWidth: 280,
