...
export default function AuthBadge({ inline = false }: { inline?: boolean }) {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        // Credential status is owner-only (it describes the operator's live
        // Claude session), so carry the console's bearer token. A visitor
        // without one gets 401 and the badge simply stays hidden.
        const tok = localStorage.getItem("buildfork_jobs_token");
        const r = await fetch(`${BASE_PATH}/api/credentials`, {
          cache: "no-store",
          headers: tok && tok !== "local" ? { Authorization: `Bearer ${tok}` } : {},
        });
        if (!r.ok) { if (alive) { setOk(null); } return; }
