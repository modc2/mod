...
//      issues an HMAC session token (server secret at ~/.mod/build-fork/terminal.secret).
...
const SECRET_PATH = path.join(os.homedir(), ".mod", "build-fork", "terminal.secret");
...
// next-server cwd is the app root (…/build-fork/src/app); config.json is two levels up.
export function ownerAddress(): string | null {
  const candidates = [
    path.resolve(process.cwd(), "../../config.json"),
    path.resolve(process.cwd(), "../config.json"),
    path.join(os.homedir(), "mod/mod/orbit/build-fork/config.json"),
    "/root/mod/mod/orbit/build-fork/config.json",
  ];
