...
const USER_HZ: u64 = 100; // clock ticks per second in /proc — fixed for userspace.
...
struct Snap {
    ...
    start_ticks: u64,  // field 22: ticks since boot when the process started
}

/// Boot time in epoch seconds, from /proc/stat's `btime` line. Process start
/// times are recorded relative to boot, so this is what turns them into wall
/// clock ("this app has been up since…").
fn boot_epoch() -> u64 {
...
fn pid_snap(pid: i64) -> Option<Snap> {
    ...
    // and starttime is field 22 → rest[19].
    ...
    Some(Snap { comm, state, ticks: utime + stime, start_ticks })
}
...
pub async fn snapshot() -> Value {
        ...
        match attribute(pid, &anchor) {
            Some((name, category)) => {
                ...
                entry.1.push(json!({
                    ...
                    "started_at": btime + snap.start_ticks / USER_HZ,
                }));
            }
            None => {
                let label = labels.get(&pid).cloned().unwrap_or(snap.comm);
    ...
    let mut apps: Vec<Value> = modules
        ...
        .map(|(name, (category, mut procs))| {
            ...
            // The app came up when its OLDEST surviving process did — a worker
            // respawned five minutes ago doesn't reset the app's uptime.
            let started_at = procs.iter().filter_map(|p| p["started_at"].as_u64()).min();
            json!({
                "module": name,
                "category": category,
                "mem_mb": mem_mb,
