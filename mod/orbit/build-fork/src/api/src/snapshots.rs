...
//! in `~/.mod/build-fork/blobs/`. Adding ipfs/bitstore/dstore later is a new enum
...
pub struct VersionRecord {
    ...
    /// `true` when `cid` addresses an encrypted bundle blob (private module,
    /// see privacy.rs) — restoring it needs the module password.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub encrypted: Option<bool>,
    /// For encrypted versions: the tree CID the plaintext would have had.
    /// Equal trees ⇒ equal plain_cid, so dedup works without decrypting
    /// (the ciphertext CID changes every time — fresh nonce per bundle).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub fn default_store() -> Store {
    let home = std::env::var("HOME").unwrap_or_else(|_| "/tmp".to_string());
    let blobs = PathBuf::from(home).join(".mod").join("build-fork").join("blobs");
...
pub fn restore_into(target: &Path, cid: &str, store: &Store) -> Result<usize, String> {
    ...
    write_manifest_files(target, &manifest, |c| store.get(c))
}

/// Safe-write core shared by every restore path (blob-store restores and
/// decrypted private-module bundles): writes `manifest`'s files into
/// `target`, fetching each body through `fetch`.
...
/// Read a module tree fully into memory: the exact (tree_cid, manifest)
/// hash_dir would produce plus a cid → bytes map of every file body. The
/// plaintext side of an encrypted (private-module) snapshot — nothing here
/// touches the blob store, so no plaintext blob ever lands on disk.
pub fn bundle_dir(
    root: &Path,
    max_bytes: u64,
) -> Result<(String, Manifest, std::collections::HashMap<String, Vec<u8>>), String> {
    let (tree_cid, manifest) = hash_dir(root, max_bytes)?;
    let mut blobs = std::collections::HashMap::new();
    for entry in &manifest.files {
        let bytes = std::fs::read(root.join(&entry.path))
            .map_err(|e| format!("read {}: {e}", entry.path))?;
        blobs.insert(entry.cid.clone(), bytes);
    }
    Ok((tree_cid, manifest, blobs))
}
...
pub fn versions_dir() -> PathBuf {
    let home = std::env::var("HOME").unwrap_or_else(|_| "/tmp".to_string());
    PathBuf::from(home).join(".mod").join("build-fork").join("versions")
}
