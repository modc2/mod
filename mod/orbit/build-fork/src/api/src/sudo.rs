...
//! (`~/.mod/build-fork/sudo_policy.json`): session length (0 = ask every time) and a
...
// Off-chain, like the whitelist: private auth state lives in ~/.mod/build-fork/.
...
mod tests {
    ...
    fn setup_owner(key: &SigningKey) -> tempdir_guard::TempHome {
        ...
        let build = home.path().join(".mod").join("build-fork");
