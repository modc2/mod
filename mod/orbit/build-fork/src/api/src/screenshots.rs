...
//! sees. Results cache under ~/.mod/build-fork/screenshots/ — fresh shots are
...
fn shots_dir() -> PathBuf {
    let dir = PathBuf::from(format!("{}/.mod/build-fork/screenshots", home()));
...
fn gateway_host() -> String {
    if let Ok(h) = std::env::var("BUILD_FORK_SHOT_HOST") {
        if !h.trim().is_empty() {
...
fn browser_bin() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("BUILD_FORK_SHOT_BROWSER") {
