...
API_PORT="${API_PORT:-8894}"
APP_PORT="${APP_PORT:-8895}"
...
PRIVATE_DIR="/home/node/.mod/build-fork"
mkdir -p "$PRIVATE_DIR"
...
PORT=\$API_PORT /app/bin/build-fork-jobs &
API_PID=\$!
echo "build-fork-jobs API on :\$API_PORT (pid \$API_PID)"
...
cd /app/src/app
NEXT_PUBLIC_BASE_PATH="/build-fork" \\
PORT=\$APP_PORT \\
HOSTNAME="0.0.0.0" \\
