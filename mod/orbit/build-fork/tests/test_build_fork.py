...
def server_is_open():
    ...
    import urllib.request
    try:
        req = urllib.request.Request('http://localhost:8894/jobs', method='GET')
...
class TestUnit:
    ...
    def test_config_loads(self):
        ...
        assert c.config['name'] == 'build-fork'
...
class TestWhitelist:
    ...
    def c(self, tmp_path):
        ...
        # Isolate the whitelist file from the real ~/.mod/build-fork/whitelist.json
...
class TestEnsureEnv:
    ...
    def test_ensure_env_skips_build_when_binary_exists(self, c, tmp_path):
        ...
        cargo.write_text('[package]\nname = "test"')
        (tmp_path / 'target' / 'release').mkdir(parents=True)
        (tmp_path / 'target' / 'release' / 'build-fork-jobs').write_text('bin')
        result = c.ensure_env(app_dir=str(tmp_path), api_dir=str(tmp_path))
        assert result['api_build']['ok'] is True
...
class TestServe:
    ...
    def test_serve_registers_in_namespace(self, c):
        ...
        api_url = f"http://localhost:{c.config.get('port', 8894)}"
        app_url = f"http://localhost:{c.config.get('app_port', 8895)}"
        mock_registry.reg.assert_called_once_with('build-fork', api_url)
        mock_registry.reg_app.assert_called_once_with(
            'build-fork', app_url,
            owner=c.key.address, api_url=api_url)

    def test_serve_sets_base_path_env(self, c):
        """serve() passes NEXT_PUBLIC_BASE_PATH=/build-fork to the Next.js app."""
        popen_calls = []
        ...
        assert app_call[0]['env']['NEXT_PUBLIC_BASE_PATH'] == '/build-fork'
        assert app_call[0]['env']['NEXT_PUBLIC_API_PORT'] == '8894'
    ...
    def test_serve_custom_ports(self, c):
        ...
        mock_registry.reg.assert_called_once_with('build-fork', 'http://localhost:9000')
        mock_registry.reg_app.assert_called_once_with(
            'build-fork', 'http://localhost:9001',
    ...
    def test_serve_uses_pm2_when_available(self, c):
        ...
        mock_registry.reg.assert_called_once_with('build-fork', 'http://localhost:8894')
