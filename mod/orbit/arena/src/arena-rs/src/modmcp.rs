...
/// The seat range a class declared: `players = 2`, or `players = [1, 4]`.
///
/// The registry reads a class with a scanner, so this is the attribute's
/// source text and not its value — `[1, SEATS]` is a name the scanner cannot
/// resolve and reads as a bare `1`, which is why the drills in the example
/// pack write both ends out. Anything unreadable is the old default of two,
/// because a wasm game keeps its seat count in `game_info` rather than here
/// and must not be opened one-handed by accident.
fn declared_seats(attributes: &Value) -> (usize, usize) {
    let raw = attributes
        .as_array()
        .and_then(|a| {
            a.iter()
    ...
    (low.clamp(1, 16), high.clamp(low.clamp(1, 16), 16))
}

async fn open_table(m: &store::WasmModule, args: &Value) -> Result<Value, String> {
    ...
    // the caller may pick anywhere inside that and nowhere outside it. A game
    // that says `players = [1, 4]` really can be opened alone — which is the
    // only way to sit one model down at a drill and read its score off the
    // table — while one that says `players = 2` is still opened two-handed
    // however few seats were asked for.
    let (low, high) = declared_seats(&info["info"]["attributes"]);
    let seats = if asked == 0 { low } else { asked.clamp(low, high) };
...
async fn play_move(args: &Value) -> Result<Value, String> {
    ...
    // Where the table stands, before anything is added to it. This costs a
    // replay, and buys the two things a caller most needs to be told: that it
    // is not your turn, and that this game wants every seat at once.
    let before = replay(&s).await?;
    let active: Vec<u64> = before["active"]
        .as_array()
        .map(|a| a.iter().filter_map(|v| v.as_u64()).collect())
        .unwrap_or_default();
    if before["done"].as_bool().unwrap_or(false) || active.is_empty() {
        return Err(format!(
            "this game is over — {}. `state` has the result, and `open` starts another.",
            before["result"]["summary"].as_str().unwrap_or("nobody is to move")
        ));
    }
    ...
    let round = match args.get("moves").and_then(|v| v.as_object()) {
        ...
        None => {
            // Unstated, the seat is whoever is to move — which is the only
            // seat it could be in a game where one moves at a time.
            let seat = match args.get("seat").and_then(|v| v.as_u64()) {
                Some(seat) => {
                    if !active.contains(&seat) {
                        return Err(format!(
                            "it is not seat {seat}'s move — {} to play. Nothing was played.",
                            active.iter().map(|s| format!("seat {s}"))
                                .collect::<Vec<_>>().join(" and ")
                        ));
                    }
                    seat
                }
                None if active.len() == 1 => active[0],
                None => {
                    return Err(format!(
                        "{} move at once in this game, so pass `moves` — {{\"0\": \"…\", \"1\": \"…\"}} \
                         — rather than one `move`. Neither seat sees the other's until both are in.",
                        active.iter().map(|s| format!("seat {s}"))
                            .collect::<Vec<_>>().join(" and ")
                    ))
                }
...
async fn ask_player(m: &store::WasmModule, args: &Value) -> Result<Value, String> {
    let view = args
        .get("view")
        .and_then(|v| v.as_str())
...
fn about(m: &store::WasmModule) -> Value {
...
pub fn module_of(key: &str) -> Result<store::WasmModule, String> {
...
fn instructions(m: &store::WasmModule) -> String {
    match m.role.as_str() {
        "game" => format!(
...
fn prompts_for(m: &store::WasmModule) -> Value {
    match m.role.as_str() {
        "game" => json!([{
            "name": "play",
...
fn prompt_text(m: &store::WasmModule, name: &str) -> Option<String> {
    match (m.role.as_str(), name) {
...
mod tests {
    ...
    fn attrs(players: &str) -> Value {
        json!([{ "name": "players", "value": players }])
    }

    #[test]
    fn a_game_that_seats_one_can_be_opened_alone() {
        // `players = [1, 4]` — a drill. One seat is the whole point of it, and
        // before this was read as a range every table opened two-handed.
        assert_eq!(declared_seats(&attrs("[1, 4]")), (1, 4));
        assert_eq!(declared_seats(&attrs("2")), (2, 2));
        // A scanner cannot resolve a name, so half a range reads as one number
        // and the game is pinned there — which is why the examples spell both
        // ends out, and why this is worth a test rather than a comment.
        assert_eq!(declared_seats(&attrs("[1, SEATS]")), (1, 1));
        // Nothing readable, including a wasm game that keeps its seat count in
        // `game_info`: the old default, so nothing is opened one-handed by
        // accident.
        assert_eq!(declared_seats(&json!([])), (2, 2));
        assert_eq!(declared_seats(&Value::Null), (2, 2));
    }
