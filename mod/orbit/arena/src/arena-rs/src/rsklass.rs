...
pub struct Class {
    ...
    pub doc: String,
    pub kind: String,
    /// `struct Bot;` — nothing to construct, so the shim can write the
    /// `Default` that makes `Bot` buildable without the author saying so.
    pub unit: bool,
...
fn scrub(text: &str) -> Vec<Scrubbed> {
    ...
    while i < b.len() {
        ...
        if c == b'\n' {
            out.push(Scrubbed { code: std::mem::take(&mut line), doc: String::new() });
    ...
    out.push(Scrubbed { code: line, doc: String::new() });
...
/// One item head: everything up to the `{`, `}` or `;` that ended it, with the
/// brace depth it opened at and the line it started on.
///
/// Working in these rather than in lines is what makes the scanner agree with
/// how Rust is actually written. `impl Bot { fn play(&self) {} }` on one line
/// holds three of them; a `fn` whose arguments are spread over five lines
/// holds one. Either way the depth says whether an item is a member of the
/// impl block or something inside a method body, which is the only question
/// this scanner has to get right.
#[derive(Debug)]
struct Seg {
    text: String,
    line: usize,
    depth: i32,
    doc: String,
    /// What ended it: `{`, `}` or `;`. The difference between
    /// `struct Bot;` and `struct Bot {` is the whole of how a unit struct is
    /// recognised, and a unit struct is how most players are written.
    ends: char,
}

fn segments(rows: &[Scrubbed]) -> Vec<Seg> {
    let mut out: Vec<Seg> = Vec::new();
    let mut buf = String::new();
    let mut start: Option<usize> = None;
    let mut depth = 0i32;

    let flush = |buf: &mut String, start: &mut Option<usize>, depth: i32, ends: char,
                     out: &mut Vec<Seg>, rows: &[Scrubbed]| {
        let text = buf.trim().to_string();
        let line = start.unwrap_or(0);
        buf.clear();
        *start = None;
        if text.is_empty() {
            return;
        ...
        out.push(Seg {
            ...
            depth,
            ends,
            doc: rows.get(line).map(|r| r.doc.clone()).unwrap_or_default(),
        });
    };

    for (i, row) in rows.iter().enumerate() {
        for c in row.code.chars() {
            match c {
                '{' => {
                    flush(&mut buf, &mut start, depth, '{', &mut out, rows);
                    depth += 1;
                }
                '}' => {
                    flush(&mut buf, &mut start, depth, '}', &mut out, rows);
        ...
        // A line break inside an item head is a space, not the end of it.
        buf.push(' ');
    }
    flush(&mut buf, &mut start, depth, ' ', &mut out, rows);
    out
...
/// The text between the first `(` and the `)` that closes it.
fn parens(text: &str) -> Option<String> {
    let open = text.find('(')?;
    let mut depth = 0i32;
    for (at, c) in text[open..].char_indices() {
        match c {
            ...
            ')' => {
                depth -= 1;
                if depth == 0 {
                    return Some(text[open + 1..open + at].trim().to_string());
                }
            }
            _ => {}
        }
...
fn scan(text: &str) -> Scan {
    ...
    for seg in &segs {
        ...
        // Left the impl block.
        if in_impl.is_some_and(|(_, body)| seg.depth < body) {
            in_impl = None;
        }

        let bare = code.trim_start_matches("pub ").trim_start();
        let bare = bare.strip_prefix("pub(crate) ").unwrap_or(bare).trim_start();
        ...
        if seg.depth == 0 {
            ...
            for keyword in ["struct", "enum"] {
                ...
                if let Some(name) = name_after(bare, keyword) {
                    declared = true;
                    // `struct Bot;` with no fields and no tuple — there is
                    // literally nothing to build, and saying so lets the shim
                    // build it without an author writing `new`.
                    let unit = seg.ends == ';' && !bare.contains('(');
                    match classes.iter_mut().find(|c| c.name == name) {
                        // An `impl` we met first put the name on the board;
                        // the declaration is where the doc comment lives.
                        Some(existing) => {
                            existing.kind = keyword.to_string();
                            existing.unit = unit;
                            if existing.doc.is_empty() {
                                existing.doc = seg.doc.clone();
                            }
                        }
            ...
            if find_word(bare, "impl") == Some(0) {
                let after = bare["impl".len()..].trim();
                // Drop a generic parameter list written right after `impl`.
                ...
                let (trait_name, type_name) = match find_word(after, "for") {
                    Some(at) => (Some(after[..at].trim().to_string()), after[at + 3..].trim()),
                    None => (None, after),
                };
                ...
                match trait_name {
                    ...
                    None => in_impl = Some((idx, seg.depth + 1)),
                }
                continue;
            }
        }
        ...
        let Some((idx, body)) = in_impl else {
            if seg.depth == 0 && bare.starts_with("fn ") {
        ...
        // `fn`, with whatever qualifiers were written in front of it.
        let mut head = bare;
        ...
        let args = parens(head).unwrap_or_default();
        let ret = head
            .rsplit_once("->")
            .map(|(_, r)| r.trim().trim_end_matches("where").trim().to_string())
            .filter(|_| head.contains("->"))
            .unwrap_or_default();
        let receiver = args
            .split(',')
            .next()
...
fn no_struct() -> &'static str {
    "no `struct` with an `impl` block in this file — the unit here is a class: a struct whose \
     ...
     `m arena/template role=game lang=rust` prints a starting point."
}

/// Will this `use` resolve, and mean anything, in the crate we are about to
/// build? Only std, core and alloc are there at all — and of those, the paths
/// that name a host this target does not have resolve and then do nothing.
fn resolves(path: &str) -> bool {
    let top = path.split("::").next().unwrap_or("");
    matches!(top, "std" | "core" | "alloc" | "self" | "super" | "crate")
        && !DEAD_PATHS.iter().any(|d| path.starts_with(d))
}
...
pub fn describe(bytes: &[u8]) -> R<Value> {
    ...
    Ok(json!({
        ...
        "exports": main.methods.iter().map(|m| json!({
            "name": m.name,
            "kind": if m.receiver.is_empty() { "associated fn" } else { "method" },
            "receiver": m.receiver,
        ...
        "attributes": main.consts.iter().map(|(k, v)| json!({ "name": k, "value": v }))
            .collect::<Vec<_>>(),
        // Which of the consts the shim actually reads were declared. Anything
        // else in the impl block is the author's business, not the harness's.
        "declares": META_CONSTS.iter().filter(|k| main.konst(k).is_some())
            .collect::<Vec<_>>(),
...
"#;
    ...
    fn consts_come_through_for_the_shim() {
        ...
        assert_eq!(describe(GAME.as_bytes()).unwrap()["declares"], json!(["NAME", "PLAYERS"]));
    ...
    fn a_struct_with_play_is_a_player() {
        ...
        assert_eq!(v["exports"][0]["kind"], "method");
        assert_eq!(v["kind"], "struct");
        assert!(chosen(src.as_bytes()).unwrap().unit, "`struct Bot;` has nothing to construct");
        assert!(!chosen(GAME.as_bytes()).unwrap().unit, "a struct with fields does");
    }
