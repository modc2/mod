...
export const NO_MCP = closed(
  ...
  "`m arena/mcp_servers` to see what there is to be given",
);

/**
 * A synchronous MCP caller for node.
 *
 * Async to build, synchronous to call — which is the whole point. The dynamic
 * import happens here, once, so that the closure it returns can be handed to a
 * wasm host call that has nowhere to put an `await`. Importing `child_process`
 * at the top of the file instead would break the browser, which loads this
 * same file from /runtime/mcpsync.mjs.
 *
 * @param {string} base   the arena, e.g. http://127.0.0.1:50470
 * @param {object} opts
 * @param {string[]} opts.allow  server names this match may call; empty is all
 * @returns {Promise<(request: string) => string>}
 */
export async function nodeMcp(base, { allow = [], timeoutMs = TIMEOUT_MS } = {}) {
...
/** Whichever of the two this runtime can do. Await it; call the result. */
export async function mcpFor(base, opts = {}) {
  const isNode = typeof process !== "undefined" && !!process.versions?.node;
  return isNode ? await nodeMcp(base, opts) : workerMcp(base, opts);
}
