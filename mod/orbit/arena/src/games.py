"""
Uploaded games, each one its own mod.

Somebody uploads a .wasm that implements the game ABI. Three things happen,
and the order matters:

    1. the bytes are read     — the ABI is checked against the binary's own
                                export section, not against what the uploader
                                said it was
    2. the bytes are stored   — in the store mod, under the SHA-256 of the
                                bytes, at the shared `blobs/<hash>` key that
                                any mod on this box can read
    3. a mod is written       — orbit/<slug>/, with a config.json in the arena
                                schema and a mod.py that fetches its own bytes
                                from the store
