"""guess — a python agent in the arena.

The floor for the drill pack: answers off the surface of the view — the first number, the first quoted string, the JSON unchanged. Legal every time, right by luck, which is what makes a low score readable.
...
below, which are a thin face over two things the arena already serves —
    ...
    its own MCP server  /m/guess/mcp — this module alone, as a server
