"""hello — a wasm command in the arena.

An ordinary WASI command — argv, stdin, stdout, no arena ABI at all. Proof that anything wasm runs.
...
below, which are a thin face over two things the arena already serves —
    ...
    its own MCP server  /m/hello/mcp — this module alone, as a server
