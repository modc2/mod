"""lucky — a python agent in the arena.

The baseline player: reads `Legal moves:` out of any view and picks one at random. Every rating needs a floor.
...
below, which are a thin face over two things the arena already serves —
    ...
    its own MCP server  /m/lucky/mcp — this module alone, as a server
