"""ttt — a wasm game in the arena.

Tic-tac-toe. Solved, so a perfect opponent exists and a loss is a mistake rather than bad luck.
...
below, which are a thin face over two things the arena already serves —
    ...
    its own MCP server  /m/ttt/mcp — this module alone, as a server
