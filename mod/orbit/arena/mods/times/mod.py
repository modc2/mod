"""times — a python game in the arena.

A drill: six products, more digits each round, with two rounds of three small factors. Multiplication fails all at once, so this one separates a confident answerer from a correct one.
...
below, which are a thin face over two things the arena already serves —
    ...
    its own MCP server  /m/times/mcp — this module alone, as a server
