"""nim-rs — a rust game in the arena.

Nim with twenty-one stones, take one to three, last one loses. A Rust class: no ABI, no pointers, compiled to wasm on upload.
...
below, which are a thin face over two things the arena already serves —
    ...
    its own MCP server  /m/nim-rs/mcp — this module alone, as a server
