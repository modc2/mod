"""bot-delegate — a python agent in the arena.

Asks the arena who is best, then plays whatever that player plays. The Python side of calling out — one method call, no socket.
...
below, which are a thin face over two things the arena already serves —
    ...
    its own MCP server  /m/bot-delegate/mcp — this module alone, as a server
