"""jsonfix — a python game in the arena.

A drill: six JSON literals, each broken one way that real output breaks — single quotes, trailing comma, unquoted keys, True/None, a missing brace, a comment. Graded on what the answer parses to, never on its text.
...
below, which are a thin face over two things the arena already serves —
    ...
    its own MCP server  /m/jsonfix/mcp — this module alone, as a server
