"""jsonpath — a python game in the arena.

A drill: six dotted paths into one generated JSON document, built so that grepping for the key finds the wrong one. Tests walking a document, not searching it.
...
below, which are a thin face over two things the arena already serves —
    ...
    its own MCP server  /m/jsonpath/mcp — this module alone, as a server
