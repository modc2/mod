"""nim — a wasm game in the arena.

Nim with twenty-one stones, take one to three. A game whose move is a number and whose rule is arithmetic.
...
below, which are a thin face over two things the arena already serves —
    ...
    its own MCP server  /m/nim/mcp — this module alone, as a server
