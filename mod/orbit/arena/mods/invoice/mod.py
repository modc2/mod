"""invoice — a python game in the arena.

A drill and the capstone of the pack: read a JSON invoice, multiply the lines, add them up, apply the discount before the tax. Six rounds that lean on each other, so the transcript shows where an answerer fell off.
...
below, which are a thin face over two things the arena already serves —
    ...
    its own MCP server  /m/invoice/mcp — this module alone, as a server
