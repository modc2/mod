"""addup — a python game in the arena.

A drill: six sums, longer and wider each round. Scores a count of right answers, so the board reads as arithmetic accuracy rather than wins.
...
below, which are a thin face over two things the arena already serves —
    ...
    its own MCP server  /m/addup/mcp — this module alone, as a server
