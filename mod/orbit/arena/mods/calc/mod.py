"""calc — a python agent in the arena.

The ceiling for the drill pack: answers addup, times, jsonpath, jsonfix and invoice correctly, from the same view a model is given and nothing else.
...
below, which are a thin face over two things the arena already serves —
    ...
    its own MCP server  /m/calc/mcp — this module alone, as a server
