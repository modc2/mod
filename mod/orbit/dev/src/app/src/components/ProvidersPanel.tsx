...
// PROVIDERS panel — polls the public /providers endpoint and renders one row
...
// per-module sleep/wake/pin controls (the backend gates them anyway).
...
export default function ProvidersPanel({ apiBase, authHeader, canControl }: Props) {
  ...
  // KILL: stop now and STAY stopped. A managed module must also be disabled,
  // or the activator resurrects it on the very next request.
  ...
  return (
    <div className="flex flex-col gap-3 font-mono text-[11px]">
                ...
                <span className="flex gap-1 shrink-0" onClick={(e) => e.stopPropagation()}>
                  {down ? (
                    <button
                      ...
                      title={act ? "Start now and re-enable auto-wake" : p.installed ? "First start — bootstraps from the module's ecosystem.config.js / start.sh" : "Start the app's pm2 processes"}
                    >
                      {busy === `${p.module}:run` ? "…" : p.installed ? "start" : "run"}
                    </button>
                  ) : (
                    <>
                      {act && (
                      ...
                      <button
                        ...
                        title={act ? "Stop and stay stopped (disables auto-wake until RUN)" : "Stop the app's pm2 processes"}
                      >
                        {busy === `${p.module}:kill` ? "…" : "kill"}
                      </button>
                    </>
                  )}
                  {act && (
      ...
      <div className="text-[9.5px] leading-relaxed" style={{ color: "var(--text-tertiary)" }}>
        ...
        {installed.length > 0 ? ` · ${installed.length} off` : ""}. Managed apps sleep after their
        idle timeout and wake on the next request; KILL stops an app until you RUN it again; START
        boots an installed app that has never run; the OOM guard evicts least-recently-used apps
