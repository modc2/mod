...
/// Modules that are pm2-registered but have no online process — killed apps.
/// They have no live pids for the /proc attribution pass to find, so without
/// this they'd vanish from the panel the moment they're stopped and could
/// never be started again from it. Attribution mirrors the /system rule:
/// a proc belongs to the module whose orbit/core dir contains its cwd, exec
/// path, or a cmdline arg.
fn stopped_pm2_modules() -> Vec<(String, String)> {
    ...
    for p in &list {
        ...
        if status == "online" {
            ...
            stopped.entry(name).or_insert(category);
        }
    }
    stopped.into_iter().filter(|(n, _)| !online.contains(n)).collect()
}
...
pub async fn providers_handler() -> impl IntoResponse {
    ...
    let present: std::collections::HashSet<String> = providers
        ...
        .collect();
    for (name, category) in stopped_pm2_modules() {
    ...
    for row in providers.iter_mut() {
        let key = row
            ...
            .unwrap_or_default()
            .to_lowercase();
        row["cid"] = cids.get(&key).map(|c| json!(c)).unwrap_or(Value::Null);
    }
