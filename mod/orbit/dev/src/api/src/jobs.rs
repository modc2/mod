...
pub struct ClaudeJob {
    ...
    /// Which backend ran the job: "claude" (CLI process) or "agent"
    /// (orbit/agent module over HTTP).
...
pub struct SubmitRequest {
    ...
    /// Backend that runs the job: "claude" (default) spawns the CLI,
    /// "agent" submits to the orbit/agent module's HTTP API.
    #[serde(default)]
...
impl ClaudeJobManager {
    ...
    pub async fn submit(&self, req: SubmitRequest) -> ClaudeJob {
        ...
        // Which backend runs this job — "claude" spawns the CLI, "agent"
        // submits to the orbit/agent module over HTTP. Unknown names fall
        // back to claude (mirrors the console, which only offers wired ones).
        ...
        let job_user = req.user_address.clone().unwrap_or_default();
        let local_mode = std::env::var("DEV_JOBS_LOCAL").unwrap_or_default() == "1";
...
fn task_cids_enabled() -> bool {
    std::env::var("DEV_TASK_CIDS").unwrap_or_default() != "0"
}
...
/// `nobody`/`nogroup` (65534), overridable via DEV_SANDBOX_UID / _GID so a
...
fn sandbox_ids() -> (u32, u32) {
    let uid = std::env::var("DEV_SANDBOX_UID")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(65534);
    let gid = std::env::var("DEV_SANDBOX_GID")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(65534);
    (uid, gid)
...
fn store_bridge_path() -> Option<String> {
    let bridge = std::env::var("DEV_STORE_BRIDGE").unwrap_or_else(|_| {
        let home = std::env::var("HOME").unwrap_or_else(|_| "/root".to_string());
        format!("{}/mod/mod/orbit/build/src/api/store_bridge.py", home)
    });
