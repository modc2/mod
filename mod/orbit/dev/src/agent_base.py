...
Siblings: agent_base_rs (Rust) implements the same contract shape.
"""
...
# ── Protocol package resolution ──────────────────────────────────────
# Every mod in the fleet ships a mod.py, so whichever directory sits first
# on sys.path decides what `import mod` means. Resolve the real protocol
# package lazily and shadow-safely (same trap and cure as shelf/src/
# protocol.py): prefer the already-imported package, else import with every
# bare-mod.py directory taken off the path. Returns None when the protocol
# is genuinely absent — callers degrade rather than crash.

_PROTOCOL = None


def _protocol():
    """The `mod` protocol package — never some module's own mod.py."""
    global _PROTOCOL
    ...
    try:
        # Drop path entries that hold a bare mod.py file — the protocol is a
        # package (mod/__init__.py), so this removes shadows and nothing else.
        sys.path = [p for p in sys.path
                    if not (Path(p or ".") / "mod.py").is_file()]
        ...
        sys.path = saved_path
        if saved_mod is not None and sys.modules.get("mod") is None:
...
class AgentContract:
    ...
    def require_owner(self, key=None):
        ...
        try:
            if hasattr(key, "address"):
                ...
                m = _protocol()
                if m is None:
                    raise RuntimeError("protocol package unavailable to resolve caller")
                addr = (m.key() if key is None else m.key(key)).address.lower()
    ...
    def cid(self) -> str:
        try:
            m = _protocol()
            return m.cid(self.NAME) if m else ""
        except Exception:
            return ""
    ...
    def submit(self, prompt: str, model: Optional[str] = None,
        ...
        """Queue a job. Spawns BINARY via the module's job server."""
        port = self._cfg.get("port")
        if not port:
    ...
    def register(self, fund_eth: float = 0.0005, key=None) -> dict:
        ...
        m = _protocol()
        if m is None:
            raise RuntimeError("protocol package unavailable")
        dev = m.mod("dev")()
