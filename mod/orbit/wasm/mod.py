"""
wasm — a marketplace for computations anyone can check.
...
step is the product; everything else exists to make it possible.
    ...
    m wasm                                  # what this box carries
    m wasm/engines                          # compute types, live and planned
    m wasm/venues                           # what each venue enforces
    m wasm/inspect path=~/thing.wasm        # read the bytes, store nothing
    m wasm/publish path=~/thing.wasm title=thing price=2
    m wasm/listings                         # the market
    m wasm/run listing=thing-a1b2 input=hi  # run it here, get a receipt
    m wasm/verify <run id>                  # replay it and attest
    m wasm/to_arena listing=snake-c3d4      # a game becomes its own mod
    m wasm/serve                            # API :50080, console :50081
...
class Mod:
    ...
    def info(self):
        ...
        return {
            ...
            'app': f'http://localhost:{APP_PORT}/wasm',
            'counts': {
    ...
    def token(self, key='test.wasm', **data):
    ...
    def _me(self, key=None) -> str:
        ...
        try:
            return self.auth(key=key or 'test.wasm').key.address
        except Exception:
            return 'local'
    ...
    def app(self, gateway: str = None):
        base = f'{gateway.rstrip("/")}/wasm' if gateway else f'http://localhost:{APP_PORT}/wasm'
        api = f'{gateway.rstrip("/")}/api/wasm' if gateway else f'http://localhost:{PORT}'
    ...
    def serve(self, port: int = PORT, app_port: int = APP_PORT, host: str = '0.0.0.0'):
        ...
        /api/wasm to one and /wasm to the other — and so a restart of
        the API doesn't take the page down with it. The console reaches the API
        through its own origin at /wasm/_api, which src/app/server.py
        ...
        if shutil.which('pm2'):
            ok = self._pm2('wasm-api', api_cmd) and self._pm2('wasm-app', app_cmd)
        else:
        ...
        return {'ok': ok, 'api': f'http://localhost:{port}',
                'app': f'http://localhost:{app_port}/wasm',
                'services': ['wasm-api', 'wasm-app']}

    def restart(self):
        if shutil.which('pm2'):
            subprocess.run(['pm2', 'restart', 'wasm-api', 'wasm-app'],
                           capture_output=True)
            return {'restarted': ['wasm-api', 'wasm-app']}
        return self.serve()

    def kill(self):
        if shutil.which('pm2'):
            subprocess.run(['pm2', 'delete', 'wasm-api', 'wasm-app'],
                           capture_output=True)
        else:
            subprocess.run(['pkill', '-f', f'uvicorn api:app --host 0.0.0.0 --port {PORT}'])
            subprocess.run(['pkill', '-f', 'wasm/src/app/server.py'])
        return {'killed': ['wasm-api', 'wasm-app']}
