#!/usr/bin/env python3
"""wasm console — the app half of the module, on its own port.

The fleet's convention is two processes per module: the API on `port` and the
console on `app_port`, so the router can send `/api/wasm` to one and
`/wasm` to the other. This is the second of those. It does two jobs and
nothing else:

    /wasm/*        the files in this directory, base path kept
    /wasm/_api/*   reverse-proxied to the API on :50080

The second one is the whole reason this is a proxy rather than a static file
server. The page pins `<base href="/wasm/">` and asks its *own origin* for
...
it will render and say the API is down, which is better than not answering.
    ...
    python3 server.py                             # :50081, API at :50080
    python3 server.py --port 8080 --api http://box:50080
    WASM_APP_PORT=8080 python3 server.py      # env works too

Arguments win over the environment, and are what `m wasm/serve` passes, so
...
API = os.environ.get('WASM_API', 'http://127.0.0.1:50080').rstrip('/')
PORT = int(os.environ.get('WASM_APP_PORT', 50081))
HOST = os.environ.get('WASM_APP_HOST', '0.0.0.0')
...
# gives up first.
TIMEOUT = float(os.environ.get('WASM_PROXY_TIMEOUT', 180))
...
class Console(BaseHTTPRequestHandler):
    ...
    def _proxy(self):
        """Forward /wasm/_api/* to the API, verbatim, both directions.
        ...
        try:
            ...
            self._json(502, {'detail': f'the wasm API at {API} is not answering ({e})'})
    ...
    def _health(self):
        try:
            ...
            api_ok = False
        self._json(200, {'ok': True, 'app': 'wasm', 'port': PORT,
...
def main():
    ...
    parser = argparse.ArgumentParser(description='the wasm console service')
    ...
    print(f'wasm console → http://localhost:{PORT}{BASE}  (api {API})', flush=True)
    try:
