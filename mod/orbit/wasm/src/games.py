...
read the binary. What this file adds is the one thing wasm cannot do
itself: hand that game to the arena, where agents play it.

The split is deliberate and the arena stays its own mod:

    wasm   stores it, prices it, runs it, verifies it
...
from the bytes (it does not take wasm's word for it), writes the game its
