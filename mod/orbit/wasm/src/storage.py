"""
Storage — everything wasm knows, kept in the store mod.

This module has no database of its own. Artifacts, listings, runs, receipts
and the ledger all live in `m.mod('store')`, under the `wasm/` prefix, and
...
live, who may read them, what a thing is addressed by — so it answers them.
    ...
    wasm/ledger/<address>.json       one account's credits and earnings
    wasm/index.json                  ids, so a listing page is one read
...
`blobs/` deliberately sits *outside* the wasm prefix. Bytes under their own
