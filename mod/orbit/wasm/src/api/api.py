"""
wasm — a marketplace for computations you can check.
...
:50081) and reaches the API through its own origin at /wasm/_api, so one
...
OWNER_PATH = Path(os.path.expanduser('~/.mod/wasm/owner.json'))

app = FastAPI(title='wasm', version=VERSION,
...
async def api_alias(request, call_next):
    """/wasm/_api/* is the same API, one path segment along.

    Behind the gateway the API answers on /api/wasm and the console on
    /wasm; on a bare port both are here. Rather than teach the page two
    ...
    if path.startswith('/wasm/_api'):
        request.scope['path'] = path[len('/wasm/_api'):] or '/'
...
if __name__ == '__main__':
    import uvicorn
    uvicorn.run(app, host='0.0.0.0', port=int(os.environ.get('PORT', 50080)))
