...
Open mode (WASM_OPEN=1) skips the gate for local development. It is off by
...
SECRET_PATH = Path(os.path.expanduser('~/.mod/wasm/server.secret'))
...
def open_mode() -> bool:
    return os.environ.get('WASM_OPEN', '') not in ('', '0', 'false')
...
def status() -> Dict[str, Any]:
    return {
        ...
        'note': ('WASM_OPEN is set — every caller is treated as signed in'
                 if open_mode() else
