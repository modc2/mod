...
m wasm/venues                           # what each venue actually enforces
m wasm/inspect path=thing.wasm          # read the bytes, store nothing
m wasm/publish path=thing.wasm title=thing price=2
m wasm/listings
m wasm/run listing=<id> input=hi seed=1 # run here, get a receipt
m wasm/verify <run id>                  # replay and attest — the mechanism
m wasm/runs status=disputed             # what didn't hold up
m wasm/grant <address> 100              # credits (the only mint)
m wasm/to_arena listing=<id>            # a game becomes its own mod
...
Two services: the API (`wasm-api`, :50080, routed at `/api/wasm`) and
the console (`wasm-app`, :50081, routed at `/wasm`). The console asks
its own origin at `/wasm/_api/*`, which the app service forwards to the
...
`POST /auth/challenge` → `POST /auth/verify`. `WASM_OPEN=1` for local dev.
...
* **`new URL(x, location.href)` ignores `<base>`.** The console is served at
  `/wasm` with a base of `/wasm/`, so location-relative resolution
