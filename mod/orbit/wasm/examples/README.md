...
```bash
m wasm/publish path=examples/montecarlo.js title="Monte Carlo pi" price=0
m wasm/run listing=<id> input=200000 seed=1     # claimed
m wasm/verify <run id>                          # verified — the replay agrees
m wasm/run listing=<id> input=200000 seed=2     # a different receipt, on purpose
```
...
m wasm/publish path=../arena/src/examples/wasm/hello.wasm title=hello
m wasm/publish path=../arena/src/examples/wasm/nim.wasm title=Nim
m wasm/to_arena listing=<the nim listing>   # → orbit/nim, its own mod
```
