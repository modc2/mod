...
m wasm/publish path=examples/montecarlo.js title="Monte Carlo pi"
m wasm/run listing=monte-carlo-pi-a1b2 input=200000 seed=1
m wasm/verify <run id>                           # → verified, or disputed
```
...
wasm/artifacts/<sha256>.json     what reading those bytes says they are
wasm/listings/<id>.json          one thing for sale
wasm/runs/<id>.json              one execution and its receipt
wasm/ledger/<address>.json       credits and earnings
```

`blobs/` sits outside the wasm prefix on purpose: bytes under their own
...
```bash
m wasm/to_arena listing=nim-c3d4
...
The arena stays its own mod. wasm stores, prices, runs and verifies; the
arena seats players, rates them and keeps the board. It re-checks the ABI from
the bytes rather than taking wasm's word for it.
...
`WASM_OPEN=1` skips the gate for local development and `GET /auth/me` says
...
src/api/api.py         the API service (FastAPI) on :50080
src/app/server.py      the app service on :50081 — the console, and /_api
...
|---|---|---|---|
| `wasm-api` | 50080 | `/api/wasm` | every endpoint below |
| `wasm-app` | 50081 | `/wasm` | the console, and `/wasm/_api/*` → the API |

The page pins `<base href="/wasm/">` and asks its *own* origin for `_api`,
