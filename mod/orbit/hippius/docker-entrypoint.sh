...
for i in $(seq 1 30); do
    if curl -sf http://localhost:$APP_PORT/$MODULE > /dev/null 2>&1; then
...
:${GATEWAY_PORT} {
    @api path /$MODULE/api /$MODULE/api/* /api/$MODULE /api/$MODULE/*
    handle @api {
...
echo "  App:     http://localhost:$APP_PORT/$MODULE"
