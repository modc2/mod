...
class DonationIncentive:
    ...
    def __init__(self, coverage_ratio: float = 0.8, burn_uid: int = 0,
        ...
        # [{'epoch': int, 'uid': int, 'amount': float}]
    ...
    def settle(self, donations: List[dict], epoch_emission: float) -> dict:
        ...
        donations: [{'uid': int, 'amount': float, 'txid': str}] — already
        verified against the whitelist and miner bindings by the validator.
        Returns weights (including the burn weight) and per-miner payouts.
        """
        self.epoch += 1
        ...
        credited = self._drain_queue(budget)

        weights: Dict[str, float] = {}
        payouts: Dict[str, float] = {}
        for uid, amount in credited.items():
            w = amount / budget if budget > 0 else 0.0
            weights[str(uid)] = round(w, 8)
            payouts[str(uid)] = round(amount / self.coverage_ratio, 8)
            t = self.totals[uid]
            t['credited'] += amount
            t['projected_payout'] += amount / self.coverage_ratio
        ...
        return {
            ...
            'weights': weights,
            'payouts': payouts,
            'payout_per_tao': round(1.0 / self.coverage_ratio, 8),
        }

    def _drain_queue(self, budget: float) -> Dict[int, float]:
        ...
        for ep in epochs:
            ...
            batch_total = sum(q['amount'] for q in batch)
            if remaining <= 1e-12:
                still_queued.extend(batch)
                continue
            scale = min(1.0, remaining / batch_total) if batch_total > 0 else 0.0
            for q in batch:
    ...
    def projection(self, donation: float, epoch_emission: float) -> dict:
        """What a miner gets back for donating `donation` TAO, and how fast."""
        donation = float(donation)
        budget = float(epoch_emission) * self.coverage_ratio
        payout = donation / self.coverage_ratio
        ...
        return {
            ...
            'margin_pct': round((1.0 / self.coverage_ratio - 1.0) * 100, 4),
